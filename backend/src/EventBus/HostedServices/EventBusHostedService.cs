...
using Fast.EventBus.Wrappers;
using Fast.IaaS.Utils;
using Microsoft.Extensions.DependencyInjection;
using Microsoft.Extensions.Hosting;
using Microsoft.Extensions.Logging;

namespace Fast.EventBus.HostedServices;
...
internal sealed class EventBusHostedService : BackgroundService
{
    ...
    private async Task BackgroundProcessing(CancellationToken stoppingToken)
    {
        ...
        var eventHandlersThatShouldRun = _eventHandlers.Where(t => t.Key.ShouldRun(eventSource.EventId))
            .OrderByDescending(u => u.Value.Order).Select(u => u.Key);
            ...
            taskFactory.StartNew(async () =>
            {
                ...
                try
                {
                    ...
                    await RetryUtil.InvokeAsync(
                        async () => { await eventHandlerThatShouldRun.Handler!(eventHandlerExecutingContext); },
                ...
                catch (Exception ex)
                {
                    ...
                    // 标记异常
                    executionException =
                        new InvalidOperationException(string.Format("Error occurred executing {0}.", eventSource.EventId), ex);
        ...
        else if (subscribeOperateSource.Operate == EventSubscribeOperates.Remove)
        {
            ...
            foreach (var wrapper in _eventHandlers.Keys)
            {
                ...
                Log(LogLevel.Warning, "Subscriber<{Name}> with event ID <{EventId}> was remove.",
                    new[] {wrapper.HandlerMethod?.Name, eventId});
            }
        }
    }
