...
public static class WebApplicationBuilderExtension
{
    ...
    public static WebApplicationBuilder AddHostInjection(this WebApplicationBuilder builder)
    {
        ...
        // 优先注入框架内部的服务
        foreach (var hostInjectionType in IaaSContext.HostInjectionTypes)
        {
            var hostInjection = Activator.CreateInstance(hostInjectionType) as IHostInjection;
            hostInjection?.Configure(webHostBuilder);
        }

        var iHostInjectionType = typeof(IHostInjection);

        // 查找所有继承了 IHostInjection 的类型，并且不包含在 HostInjectionTypes 中的
        var hostInjectionTypes = IaaSContext.EffectiveTypes.Where(wh =>
            iHostInjectionType.IsAssignableFrom(wh) && wh.IsClass && !wh.IsInterface && !wh.IsAbstract &&
            !IaaSContext.HostInjectionTypes.Contains(wh)).Select(sl =>
        {
        ...
        foreach (var hostInjection in hostInjectionTypes)
        {
            hostInjection?.Configure(webHostBuilder);
        }
    ...
    public static WebApplicationBuilder AddControllersInjection(this WebApplicationBuilder builder)
    {
        ...
        // 优先添加框架内部服务
        foreach (var controllersInjectionType in IaaSContext.ControllersInjectionTypes)
        {
            var controllersInjection = Activator.CreateInstance(controllersInjectionType) as IControllersInjection;
