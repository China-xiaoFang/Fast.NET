...
namespace Fast.Swagger.Builders;
...
public static class SwaggerDocumentBuilder
{
    ...
    private static void ConfigureActionSequence(SwaggerGenOptions swaggerGenOptions)
    {
        swaggerGenOptions.OrderActionsBy(apiDesc =>
        {
            var apiDescriptionSettings =
                apiDesc.CustomAttributes().FirstOrDefault(u => u.GetType() == typeof(ApiDescriptionSettingsAttribute)) as
    ...
    private static void ConfigureSchemaIds(SwaggerGenOptions swaggerGenOptions)
    {
        ...
        static string DefaultSchemaIdSelector(Type modelType)
        {
            ...
            {
                var prefix = modelType.GetGenericArguments().Select(genericArg => DefaultSchemaIdSelector(genericArg))
    ...
    {
        // 获取所有的控制器和动作方法
        var controllers = IaaSContext.EffectiveTypes.Where(DynamicApplicationContext.IsApiController);
        if (!controllers.Any())
        {
        ...
        // 合并所有分组
        var groupOrders = controllers.SelectMany(u => GetControllerGroups(u)).Union(actions.SelectMany(u => GetActionGroups(u)))
