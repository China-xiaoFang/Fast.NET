...
public class CSRedisClientPipe<TObject> : IDisposable
{
    private readonly CSRedisClient rds;
    ...
    private readonly ConcurrentDictionary<string, (List<int> indexes, Object<RedisClient> conn)> Conns =
        new ConcurrentDictionary<string, (List<int> indexes, Object<RedisClient> conn)>();

    private readonly Queue<Func<object, object>> Parsers = new Queue<Func<object, object>>();
    private static readonly object ConnsLock = new object();
    ...
    private CSRedisClientPipe(CSRedisClient csredis,
        ConcurrentDictionary<string, (List<int> indexes, Object<RedisClient> conn)> conns, Queue<Func<object, object>> parsers)
    {
        rds = csredis;
        Conns = conns;
        Parsers = parsers;
    ...
    private CSRedisClientPipe<TReturn> PipeCommand<TReturn>(string key, Func<Object<RedisClient>, string, TReturn> handle) =>
        PipeCommand(key, handle, null);

    private CSRedisClientPipe<TReturn> PipeCommand<TReturn>(string key, Func<Object<RedisClient>, string, TReturn> handle,
        Func<object, object> parser)
    {
        if (string.IsNullOrEmpty(key))
            throw new Exception("key 不可为空或null");
        ...
        return new CSRedisClientPipe<TReturn>(rds, Conns, Parsers);
    }
    ...
    /// <returns></returns>
    public CSRedisClientPipe<object> Eval(string script, string key, params object[] args) =>
        PipeCommand(key,
            (c, k) => c.Value.Eval(script, new[] {k}, args?.Select(z => rds.SerializeRedisValueInternal(z)).ToArray()));
    ...
    /// <param name="args">参数</param>
    /// <returns></returns>
    public CSRedisClientPipe<object> EvalSHA(string sha1, string key, params object[] args) =>
        PipeCommand(key,
            (c, k) => c.Value.EvalSHA(sha1, new[] {k}, args?.Select(z => rds.SerializeRedisValueInternal(z)).ToArray()));
    ...
    {
        var msgid = rds.HIncrBy("csredisclient:Publish:msgid", channel);
    ...
    /// <param name="message">消息文本</param>
    /// <returns></returns>
    public CSRedisClientPipe<long> PublishNoneMessageId(string channel, string message) =>
        PipeCommand(channel, (c, k) => c.Value.Publish(channel, message));
    ...
    /// <returns></returns>
    public CSRedisClientPipe<bool> PfAdd<T>(string key, params T[] elements) =>
        PipeCommand(key, (c, k) => c.Value.PfAdd(k, elements?.Select(z => rds.SerializeRedisValueInternal(z)).ToArray()));
    ...
    public CSRedisClientPipe<(string member, decimal score)[]> ZPopMax(string key, long count) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZPopMax(k, count);
            return default((string, decimal)[]);
        }, obj => ((Tuple<string, decimal>[]) obj).Select(a => (a.Item1, a.Item2)).ToArray());
    ...
    public CSRedisClientPipe<(T member, decimal score)[]> ZPopMax<T>(string key, long count) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZPopMaxBytes(k, count);
            return default((T member, decimal score)[]);
        }, obj => rds.DeserializeRedisValueTuple1Internal<T, decimal>((Tuple<byte[], decimal>[]) obj));
    ...
    public CSRedisClientPipe<long> ZAdd(string key, params (decimal, object)[] scoreMembers)
    {
        if (scoreMembers == null || scoreMembers.Any() == false)
            throw new Exception("scoreMembers 参数不可为空");
        var ms = scoreMembers.Select(a => new Tuple<decimal, object>(a.Item1, rds.SerializeRedisValueInternal(a.Item2)))
            .ToArray();
    ...
    /// <returns></returns>
    public CSRedisClientPipe<long> ZCount(string key, decimal min, decimal max) =>
        PipeCommand(key,
    ...
    /// <returns></returns>
    public CSRedisClientPipe<long> ZCount(string key, string min, string max) =>
        PipeCommand(key, (c, k) => c.Value.ZCount(k, min, max));
    ...
    /// <returns></returns>
    public CSRedisClientPipe<decimal> ZIncrBy(string key, string member, decimal increment = 1) =>
        PipeCommand(key, (c, k) => c.Value.ZIncrBy(k, increment, member));
    ...
    /// <returns></returns>
    public CSRedisClientPipe<long> ZInterStore(string destination, decimal[] weights, RedisAggregate aggregate,
        params string[] keys)
    {
        ...
        if (IsMultiNode)
            throw new Exception("ZInterStore 管道命令，在分区模式下不可用");
        var prefix = Nodes.First().Value.Prefix;
        return PipeCommand(destination,
            (c, k) => c.Value.ZInterStore(k, weights, aggregate, keys.Select(z => prefix + z).ToArray()));
    }
    ...
    /// <returns></returns>
    public CSRedisClientPipe<string[]> ZRange(string key, long start, long stop) =>
        PipeCommand(key, (c, k) => c.Value.ZRange(k, start, stop));
    ...
    public CSRedisClientPipe<T[]> ZRange<T>(string key, long start, long stop) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZRangeBytes(k, start, stop);
            return default(T[]);
        }, obj => rds.DeserializeRedisValueArrayInternal<T>((byte[][]) obj));
    ...
    public CSRedisClientPipe<(string member, decimal score)[]> ZRangeWithScores(string key, long start, long stop) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZRangeWithScores(k, start, stop);
            return default((string, decimal)[]);
        }, obj => ((Tuple<string, decimal>[]) obj).Select(a => (a.Item1, a.Item2)).ToArray());
    ...
    public CSRedisClientPipe<(T member, decimal score)[]> ZRangeWithScores<T>(string key, long start, long stop) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZRangeBytesWithScores(k, start, stop);
            return default((T member, decimal score)[]);
        }, obj => rds.DeserializeRedisValueTuple1Internal<T, decimal>((Tuple<byte[], decimal>[]) obj));
    ...
    public CSRedisClientPipe<T[]> ZRangeByScore<T>(string key, decimal min, decimal max, long? count = null, long offset = 0) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZRangeBytesByScore(k, min == decimal.MinValue ? "-inf" : min.ToString(),
                max == decimal.MaxValue ? "+inf" : max.ToString(), false, offset, count);
            return default(T[]);
        }, obj => rds.DeserializeRedisValueArrayInternal<T>((byte[][]) obj));
    ...
    public CSRedisClientPipe<T[]> ZRangeByScore<T>(string key, string min, string max, long? count = null, long offset = 0) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZRangeBytesByScore(k, min, max, false, offset, count);
            return default(T[]);
        }, obj => rds.DeserializeRedisValueArrayInternal<T>((byte[][]) obj));
    ...
    /// <param name="offset">返回条件偏移位置</param>
    /// <returns></returns>
    public CSRedisClientPipe<(string member, decimal score)[]> ZRangeByScoreWithScores(string key, decimal min, decimal max,
        long? count = null, long offset = 0) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZRangeByScoreWithScores(k, min == decimal.MinValue ? "-inf" : min.ToString(),
                max == decimal.MaxValue ? "+inf" : max.ToString(), offset, count);
            return default((string, decimal)[]);
        }, obj => ((Tuple<string, decimal>[]) obj).Select(z => (z.Item1, z.Item2)));
    ...
    /// <param name="offset">返回条件偏移位置</param>
    /// <returns></returns>
    public CSRedisClientPipe<(T member, decimal score)[]> ZRangeByScoreWithScores<T>(string key, decimal min, decimal max,
        long? count = null, long offset = 0) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZRangeBytesByScoreWithScores(k, min == decimal.MinValue ? "-inf" : min.ToString(),
                max == decimal.MaxValue ? "+inf" : max.ToString(), offset, count);
            return default((T, decimal)[]);
        }, obj => rds.DeserializeRedisValueTuple1Internal<T, decimal>((Tuple<byte[], decimal>[]) obj));
    ...
    /// <param name="offset">返回条件偏移位置</param>
    /// <returns></returns>
    public CSRedisClientPipe<(string member, decimal score)[]> ZRangeByScoreWithScores(string key, string min, string max,
        long? count = null, long offset = 0) =>
        PipeCommand(key,
            (c, k) => c.Value.ZRangeByScoreWithScores(k, min, max, offset, count).Select(z => (z.Item1, z.Item2)).ToArray());
    ...
    /// <param name="offset">返回条件偏移位置</param>
    /// <returns></returns>
    public CSRedisClientPipe<(T member, decimal score)[]> ZRangeByScoreWithScores<T>(string key, string min, string max,
        long? count = null, long offset = 0) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZRangeBytesByScoreWithScores(k, min, max, offset, count);
            return default((T, decimal)[]);
        }, obj => rds.DeserializeRedisValueTuple1Internal<T, decimal>((Tuple<byte[], decimal>[]) obj));
    ...
    /// <param name="member">成员</param>
    /// <returns></returns>
    public CSRedisClientPipe<long?> ZRank(string key, object member) =>
        PipeCommand(key, (c, k) => c.Value.ZRank(k, rds.SerializeRedisValueInternal(member)));
    ...
    public CSRedisClientPipe<long> ZRem<T>(string key, params T[] member)
    {
        if (member == null || member.Any() == false)
            throw new Exception("member 参数不可为空");
    ...
    /// <param name="stop">结束位置，0表示第一个元素，-1表示最后一个元素</param>
    /// <returns></returns>
    public CSRedisClientPipe<long> ZRemRangeByRank(string key, long start, long stop) =>
        PipeCommand(key, (c, k) => c.Value.ZRemRangeByRank(k, start, stop));
    ...
    /// <param name="max">分数最大值 decimal.MaxValue 10</param>
    /// <returns></returns>
    public CSRedisClientPipe<long> ZRemRangeByScore(string key, decimal min, decimal max) =>
        PipeCommand(key,
    ...
    /// <param name="max">分数最大值 +inf (10 10</param>
    /// <returns></returns>
    public CSRedisClientPipe<long> ZRemRangeByScore(string key, string min, string max) =>
        PipeCommand(key, (c, k) => c.Value.ZRemRangeByScore(k, min, max));
    ...
    /// <param name="stop">结束位置，0表示第一个元素，-1表示最后一个元素</param>
    /// <returns></returns>
    public CSRedisClientPipe<string[]> ZRevRange(string key, long start, long stop) =>
        PipeCommand(key, (c, k) => c.Value.ZRevRange(k, start, stop));
    ...
    public CSRedisClientPipe<T[]> ZRevRange<T>(string key, long start, long stop) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZRevRangeBytes(k, start, stop);
            return default(T[]);
        }, obj => rds.DeserializeRedisValueArrayInternal<T>((byte[][]) obj));
    ...
    public CSRedisClientPipe<(string member, decimal score)[]> ZRevRangeWithScores(string key, long start, long stop) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZRevRangeWithScores(k, start, stop);
            return default((string, decimal)[]);
        }, obj => ((Tuple<string, decimal>[]) obj).Select(a => (a.Item1, a.Item2)).ToArray());
    ...
    public CSRedisClientPipe<(T member, decimal score)[]> ZRevRangeWithScores<T>(string key, long start, long stop) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZRevRangeBytesWithScores(k, start, stop);
            return default((T, decimal)[]);
        }, obj => rds.DeserializeRedisValueTuple1Internal<T, decimal>((Tuple<byte[], decimal>[]) obj));
    ...
    /// <param name="offset">返回条件偏移位置</param>
    /// <returns></returns>
    public CSRedisClientPipe<string[]>
        ZRevRangeByScore(string key, decimal max, decimal min, long? count = null, long? offset = 0) =>
        PipeCommand(key,
    ...
    /// <param name="offset">返回条件偏移位置</param>
    /// <returns></returns>
    public CSRedisClientPipe<T[]>
        ZRevRangeByScore<T>(string key, decimal max, decimal min, long? count = null, long offset = 0) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZRevRangeBytesByScore(k, max == decimal.MaxValue ? "+inf" : max.ToString(),
                min == decimal.MinValue ? "-inf" : min.ToString(), false, offset, count);
            return default(T[]);
        }, obj => rds.DeserializeRedisValueArrayInternal<T>((byte[][]) obj));
    ...
    /// <returns></returns>
    public CSRedisClientPipe<string[]>
        ZRevRangeByScore(string key, string max, string min, long? count = null, long? offset = 0) =>
    ...
    public CSRedisClientPipe<T[]> ZRevRangeByScore<T>(string key, string max, string min, long? count = null, long offset = 0) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZRevRangeBytesByScore(k, max, min, false, offset, count);
            return default(T[]);
        }, obj => rds.DeserializeRedisValueArrayInternal<T>((byte[][]) obj));
    ...
    /// <param name="offset">返回条件偏移位置</param>
    /// <returns></returns>
    public CSRedisClientPipe<(string member, decimal score)[]> ZRevRangeByScoreWithScores(string key, decimal max, decimal min,
        long? count = null, long offset = 0) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZRevRangeByScoreWithScores(k, max == decimal.MaxValue ? "+inf" : max.ToString(),
                min == decimal.MinValue ? "-inf" : min.ToString(), offset, count);
            return default((string member, decimal score)[]);
        }, obj => ((Tuple<string, decimal>[]) obj).Select(z => (z.Item1, z.Item2)).ToArray());
    ...
    /// <param name="offset">返回条件偏移位置</param>
    /// <returns></returns>
    public CSRedisClientPipe<(T member, decimal score)[]> ZRevRangeByScoreWithScores<T>(string key, decimal max, decimal min,
        long? count = null, long offset = 0) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZRevRangeBytesByScoreWithScores(k, max == decimal.MaxValue ? "+inf" : max.ToString(),
                min == decimal.MinValue ? "-inf" : min.ToString(), offset, count);
            return default((T member, decimal score)[]);
        }, obj => rds.DeserializeRedisValueTuple1Internal<T, decimal>((Tuple<byte[], decimal>[]) obj));
    ...
    /// <param name="offset">返回条件偏移位置</param>
    /// <returns></returns>
    public CSRedisClientPipe<(string member, decimal score)[]> ZRevRangeByScoreWithScores(string key, string max, string min,
        long? count = null, long offset = 0) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZRevRangeByScoreWithScores(k, max, min, offset, count);
            return default((string, decimal)[]);
        }, obj => ((Tuple<string, decimal>[]) obj).Select(z => (z.Item1, z.Item2)).ToArray());
    ...
    /// <param name="offset">返回条件偏移位置</param>
    /// <returns></returns>
    public CSRedisClientPipe<(T member, decimal score)[]> ZRevRangeByScoreWithScores<T>(string key, string max, string min,
        long? count = null, long offset = 0) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZRevRangeBytesByScoreWithScores(k, max, min, offset, count);
            return default((T, decimal)[]);
        }, obj => rds.DeserializeRedisValueTuple1Internal<T, decimal>((Tuple<byte[], decimal>[]) obj));
    ...
    /// <param name="member">成员</param>
    /// <returns></returns>
    public CSRedisClientPipe<long?> ZRevRank(string key, object member) =>
        PipeCommand(key, (c, k) => c.Value.ZRevRank(k, rds.SerializeRedisValueInternal(member)));
    ...
    /// <param name="member">成员</param>
    /// <returns></returns>
    public CSRedisClientPipe<decimal?> ZScore(string key, object member) =>
        PipeCommand(key, (c, k) => c.Value.ZScore(k, rds.SerializeRedisValueInternal(member)));
    ...
    /// <param name="keys">一个或多个有序集合，不含prefix前辍</param>
    /// <returns></returns>
    public CSRedisClientPipe<long> ZUnionStore(string destination, decimal[] weights, RedisAggregate aggregate,
        params string[] keys)
    {
        if (keys == null || keys.Length == 0)
            throw new Exception("keys 参数不可为空");
        if (weights != null && weights.Length != keys.Length)
            throw new Exception("weights 和 keys 参数长度必须相同");
        if (IsMultiNode)
            throw new Exception("ZUnionStore 管道命令，在分区模式下不可用");
        var prefix = Nodes.First().Value.Prefix;
        return PipeCommand(destination,
            (c, k) => c.Value.ZUnionStore(k, weights, aggregate, keys.Select(z => prefix + z).ToArray()));
    }
    ...
    /// <param name="count">数量</param>
    /// <returns></returns>
    public CSRedisClientPipe<RedisScan<(string member, decimal score)>> ZScan(string key, long cursor, string pattern = null,
        long? count = null) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZScan(k, cursor, pattern, count);
            return default(RedisScan<(string, decimal)>);
        }, obj =>
        {
    ...
    /// <param name="count">数量</param>
    /// <returns></returns>
    public CSRedisClientPipe<RedisScan<(T member, decimal score)>> ZScan<T>(string key, long cursor, string pattern = null,
        long? count = null) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZScanBytes(k, cursor, pattern, count);
            return default(RedisScan<(T, decimal)>);
        }, obj =>
        {
    ...
    public CSRedisClientPipe<T[]> ZRangeByLex<T>(string key, string min, string max, long? count = null, long offset = 0) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.ZRangeBytesByLex(k, min, max, offset, count);
            return default(T[]);
        }, obj => rds.DeserializeRedisValueArrayInternal<T>((byte[][]) obj));
    ...
    public CSRedisClientPipe<long> SAdd<T>(string key, params T[] members)
    {
        if (members == null || members.Any() == false)
            throw new Exception("members 参数不可为空");
    ...
    public CSRedisClientPipe<string[]> SDiff(params string[] keys)
    {
        if (keys == null || keys.Length == 0)
            throw new Exception("keys 参数不可为空");
        if (IsMultiNode)
            throw new Exception("SDiff 管道命令，在分区模式下不可用");
        var prefix = Nodes.First().Value.Prefix;
    ...
    public CSRedisClientPipe<T[]> SDiff<T>(params string[] keys)
    {
        if (keys == null || keys.Length == 0)
            throw new Exception("keys 参数不可为空");
        if (IsMultiNode)
            throw new Exception("SDiff<T> 管道命令，在分区模式下不可用");
        var prefix = Nodes.First().Value.Prefix;
        return PipeCommand(keys.First(), (c, k) =>
        {
            c.Value.SDiffBytes(keys.Select(z => prefix + z).ToArray());
            return default(T[]);
        }, obj => rds.DeserializeRedisValueArrayInternal<T>((byte[][]) obj));
    }
    ...
    public CSRedisClientPipe<long> SDiffStore(string destination, params string[] keys)
    {
        if (string.IsNullOrEmpty(destination))
            throw new Exception("destination 参数不可为空");
        if (keys == null || keys.Length == 0)
            throw new Exception("keys 参数不可为空");
        if (IsMultiNode)
            throw new Exception("SDiffStore 管道命令，在分区模式下不可用");
        var prefix = Nodes.First().Value.Prefix;
    ...
    public CSRedisClientPipe<string[]> SInter(params string[] keys)
    {
        if (keys == null || keys.Length == 0)
            throw new Exception("keys 参数不可为空");
        if (IsMultiNode)
            throw new Exception("SInter 管道命令，在分区模式下不可用");
        var prefix = Nodes.First().Value.Prefix;
    ...
    public CSRedisClientPipe<T[]> SInter<T>(params string[] keys)
    {
        if (keys == null || keys.Length == 0)
            throw new Exception("keys 参数不可为空");
        if (IsMultiNode)
            throw new Exception("SInter<T> 管道命令，在分区模式下不可用");
        var prefix = Nodes.First().Value.Prefix;
        return PipeCommand(keys.First(), (c, k) =>
        {
            c.Value.SInterBytes(keys.Select(z => prefix + z).ToArray());
            return default(T[]);
        }, obj => rds.DeserializeRedisValueArrayInternal<T>((byte[][]) obj));
    }
    ...
    public CSRedisClientPipe<long> SInterStore(string destination, params string[] keys)
    {
        ...
        if (keys == null || keys.Length == 0)
            throw new Exception("keys 参数不可为空");
        if (IsMultiNode)
            throw new Exception("SInterStore 管道命令，在分区模式下不可用");
        var prefix = Nodes.First().Value.Prefix;
    ...
    /// <param name="member">成员</param>
    /// <returns></returns>
    public CSRedisClientPipe<bool> SIsMember(string key, object member) =>
        PipeCommand(key, (c, k) => c.Value.SIsMember(k, rds.SerializeRedisValueInternal(member)));
    ...
    /// <param name="key">不含prefix前辍</param>
    /// <returns></returns>
    public CSRedisClientPipe<T[]> SMembers<T>(string key) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.SMembersBytes(k);
            return default(T[]);
        }, obj => rds.DeserializeRedisValueArrayInternal<T>((byte[][]) obj));
    ...
    {
        if (IsMultiNode)
            throw new Exception("SMove 管道命令，在分区模式下不可用");
        return PipeCommand(source,
            (c, k) => c.Value.SMove(k, (c.Pool as RedisClientPool)?.Prefix + destination,
                rds.SerializeRedisValueInternal(member)));
    }
    ...
    /// <param name="key">不含prefix前辍</param>
    /// <returns></returns>
    public CSRedisClientPipe<T> SPop<T>(string key) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.SPopBytes(k);
            return default(T);
        }, obj => rds.DeserializeRedisValueInternal<T>((byte[]) obj));
    ...
    /// <param name="key">不含prefix前辍</param>
    /// <returns></returns>
    public CSRedisClientPipe<T> SRandMember<T>(string key) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.SRandMemberBytes(k);
            return default(T);
        }, obj => rds.DeserializeRedisValueInternal<T>((byte[]) obj));
    ...
    /// <returns></returns>
    public CSRedisClientPipe<string[]> SRandMembers(string key, int count = 1) =>
        PipeCommand(key, (c, k) => c.Value.SRandMembers(k, count));
    ...
    /// <param name="count">返回个数</param>
    /// <returns></returns>
    public CSRedisClientPipe<T[]> SRandMembers<T>(string key, int count = 1) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.SRandMembersBytes(k, count);
            return default(T[]);
        }, obj => rds.DeserializeRedisValueArrayInternal<T>((byte[][]) obj));
    ...
    public CSRedisClientPipe<string[]> SUnion(params string[] keys)
    {
        if (keys == null || keys.Length == 0)
            throw new Exception("keys 参数不可为空");
        if (IsMultiNode)
            throw new Exception("SUnion 管道命令，在分区模式下不可用");
        var prefix = Nodes.First().Value.Prefix;
    ...
    public CSRedisClientPipe<T[]> SUnion<T>(params string[] keys)
    {
        if (keys == null || keys.Length == 0)
            throw new Exception("keys 参数不可为空");
        if (IsMultiNode)
            throw new Exception("SUnion<T> 管道命令，在分区模式下不可用");
        var prefix = Nodes.First().Value.Prefix;
        return PipeCommand(keys.First(), (c, k) =>
        {
            c.Value.SUnionBytes(keys.Select(z => prefix + z).ToArray());
            return default(T[]);
        }, obj => rds.DeserializeRedisValueArrayInternal<T>((byte[][]) obj));
    }
    ...
    public CSRedisClientPipe<long> SUnionStore(string destination, params string[] keys)
    {
        ...
        if (keys == null || keys.Length == 0)
            throw new Exception("keys 参数不可为空");
        if (IsMultiNode)
            throw new Exception("SUnionStore 管道命令，在分区模式下不可用");
        var prefix = Nodes.First().Value.Prefix;
    ...
    public CSRedisClientPipe<RedisScan<T>> SScan<T>(string key, long cursor, string pattern = null, long? count = null) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.SScanBytes(k, cursor, pattern, count);
            return default(RedisScan<T>);
        }, obj =>
        {
    ...
    /// <param name="index">索引</param>
    /// <returns></returns>
    public CSRedisClientPipe<T> LIndex<T>(string key, long index) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.LIndexBytes(k, index);
            return default(T);
        }, obj => rds.DeserializeRedisValueInternal<T>((byte[]) obj));
    ...
    /// <returns></returns>
    public CSRedisClientPipe<long> LInsertBefore(string key, string pivot, object value) =>
        PipeCommand(key, (c, k) => c.Value.LInsert(k, RedisInsert.Before, pivot, rds.SerializeRedisValueInternal(value)));
    ...
    /// <param name="value">新元素</param>
    /// <returns></returns>
    public CSRedisClientPipe<long> LInsertAfter(string key, string pivot, object value) =>
        PipeCommand(key, (c, k) => c.Value.LInsert(k, RedisInsert.After, pivot, rds.SerializeRedisValueInternal(value)));
    ...
    /// <param name="key">不含prefix前辍</param>
    /// <returns></returns>
    public CSRedisClientPipe<T> LPop<T>(string key) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.LPopBytes(k);
            return default(T);
    ...
    public CSRedisClientPipe<long> LPush<T>(string key, params T[] value)
    {
        if (value == null || value.Any() == false)
            throw new Exception("value 参数不可为空");
        ;
    ...
    public CSRedisClientPipe<long> LPushX(string key, object value) =>
        PipeCommand(key, (c, k) => c.Value.LPushX(k, rds.SerializeRedisValueInternal(value)));
    ...
    /// <param name="stop">结束位置，0表示第一个元素，-1表示最后一个元素</param>
    /// <returns></returns>
    public CSRedisClientPipe<string[]> LRange(string key, long start, long stop) =>
        PipeCommand(key, (c, k) => c.Value.LRange(k, start, stop));
    ...
    /// <param name="stop">结束位置，0表示第一个元素，-1表示最后一个元素</param>
    /// <returns></returns>
    public CSRedisClientPipe<T[]> LRange<T>(string key, long start, long stop) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.LRangeBytes(k, start, stop);
            return default(T[]);
        }, obj => rds.DeserializeRedisValueArrayInternal<T>((byte[][]) obj));
    ...
    /// <returns></returns>
    public CSRedisClientPipe<long> LRem(string key, long count, object value) =>
        PipeCommand(key, (c, k) => c.Value.LRem(k, count, rds.SerializeRedisValueInternal(value)));
    ...
    /// <param name="value">值</param>
    /// <returns></returns>
    public CSRedisClientPipe<bool> LSet(string key, long index, object value) =>
        PipeCommand(key, (c, k) =>
        {
    ...
    /// <param name="stop">结束位置，0表示第一个元素，-1表示最后一个元素</param>
    /// <returns></returns>
    public CSRedisClientPipe<bool> LTrim(string key, long start, long stop) =>
        PipeCommand(key, (c, k) =>
        {
    ...
    /// <param name="key">不含prefix前辍</param>
    /// <returns></returns>
    public CSRedisClientPipe<T> RPop<T>(string key) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.RPopBytes(k);
            return default(T);
    ...
    {
        if (IsMultiNode)
            throw new Exception("RPopLPush 管道命令，在分区模式下不可用");
    ...
    {
        if (IsMultiNode)
            throw new Exception("RPopLPush<T> 管道命令，在分区模式下不可用");
        return PipeCommand(source, (c, k) =>
        {
            c.Value.RPopBytesLPush(k, (c.Pool as RedisClientPool)?.Prefix + destination);
            return default(T);
        }, obj => rds.DeserializeRedisValueInternal<T>((byte[]) obj));
    }
    ...
    public CSRedisClientPipe<long> RPushX(string key, object value) =>
        PipeCommand(key, (c, k) => c.Value.RPushX(k, rds.SerializeRedisValueInternal(value)));
    ...
    public CSRedisClientPipe<long> HDel(string key, params string[] fields)
    {
        if (fields == null || fields.Any() == false)
            throw new Exception("fields 参数不可为空");
    ...
    /// <param name="key">不含prefix前辍</param>
    /// <returns></returns>
    public CSRedisClientPipe<Dictionary<string, T>> HGetAll<T>(string key) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.HGetAllBytes(k);
            return default(Dictionary<string, T>);
        }, obj => rds.DeserializeRedisValueDictionaryInternal<string, T>((Dictionary<string, byte[]>) obj));
    ...
    /// <returns></returns>
    public CSRedisClientPipe<long> HIncrBy(string key, string field, long value = 1) =>
        PipeCommand(key, (c, k) => c.Value.HIncrBy(k, field, value));
    ...
    /// <param name="value">增量值(默认=1)</param>
    /// <returns></returns>
    public CSRedisClientPipe<decimal> HIncrByFloat(string key, string field, decimal value) =>
        PipeCommand(key, (c, k) => c.Value.HIncrByFloat(k, field, value));
    ...
    public CSRedisClientPipe<bool> HMSet(string key, params object[] keyValues)
    {
        if (keyValues == null || keyValues.Any() == false)
            throw new Exception("keyValues 参数不可为空");
    ...
    public CSRedisClientPipe<bool> HSet(string key, string field, object value) =>
        PipeCommand(key, (c, k) => c.Value.HSet(k, field, rds.SerializeRedisValueInternal(value)));
    ...
    /// <returns></returns>
    public CSRedisClientPipe<bool> HSetNx(string key, string field, object value) =>
        PipeCommand(key, (c, k) => c.Value.HSetNx(k, field, rds.SerializeRedisValueInternal(value)));
    ...
    /// <param name="key">不含prefix前辍</param>
    /// <returns></returns>
    public CSRedisClientPipe<T[]> HVals<T>(string key) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.HValsBytes(k);
            return default(T[]);
        }, obj => rds.DeserializeRedisValueArrayInternal<T>((byte[][]) obj));
    ...
    /// <param name="count">数量</param>
    /// <returns></returns>
    public CSRedisClientPipe<RedisScan<(string field, string value)>> HScan(string key, long cursor, string pattern = null,
        long? count = null) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.HScan(k, cursor, pattern, count);
            return default(RedisScan<(string, string)>);
        }, obj =>
        {
    ...
    /// <param name="count">数量</param>
    /// <returns></returns>
    public CSRedisClientPipe<RedisScan<(string field, T value)>> HScan<T>(string key, long cursor, string pattern = null,
        long? count = null) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.HScanBytes(k, cursor, pattern, count);
            return default(RedisScan<(string, T)>);
        }, obj =>
        {
            var scan = (RedisScan<Tuple<string, byte[]>>) obj;
            return new RedisScan<(string, T)>(scan.Cursor,
                scan.Items.Select(z => (z.Item1, rds.DeserializeRedisValueInternal<T>(z.Item2))).ToArray());
        });
    ...
    public CSRedisClientPipe<long> Append(string key, object value) =>
        PipeCommand(key, (c, k) => c.Value.Append(k, rds.SerializeRedisValueInternal(value)));
    ...
    /// <returns></returns>
    public CSRedisClientPipe<long> BitCount(string key, long start, long end) =>
        PipeCommand(key, (c, k) => c.Value.BitCount(k, start, end));
    ...
    public CSRedisClientPipe<long> BitOp(RedisBitOp op, string destKey, params string[] keys)
    {
        ...
        if (IsMultiNode)
            throw new Exception("BitOp 管道命令，在分区模式下不可用");
        var prefix = Nodes.First().Value.Prefix;
    ...
    public CSRedisClientPipe<long> BitPos(string key, bool bit, long? start = null, long? end = null) =>
        PipeCommand(key, (c, k) => c.Value.BitPos(k, bit, start, end));
    ...
    /// <returns></returns>
    public CSRedisClientPipe<string> GetRange(string key, long start, long end) =>
        PipeCommand(key, (c, k) => c.Value.GetRange(k, start, end));
    ...
    /// <param name="end">结束位置，0表示第一个元素，-1表示最后一个元素</param>
    /// <returns></returns>
    public CSRedisClientPipe<T> GetRange<T>(string key, long start, long end) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.GetRangeBytes(k, start, end);
            return default(T);
        }, obj => rds.DeserializeRedisValueInternal<T>((byte[]) obj));
    ...
    /// <param name="value">字符串</param>
    /// <returns></returns>
    public CSRedisClientPipe<string> GetSet(string key, object value) =>
        PipeCommand(key, (c, k) => c.Value.GetSet(k, rds.SerializeRedisValueInternal(value)));
    ...
    /// <param name="value">值</param>
    /// <returns></returns>
    public CSRedisClientPipe<T> GetSet<T>(string key, object value) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.GetSetBytes(k, rds.SerializeRedisValueInternal(value));
            return default(T);
        }, obj => rds.DeserializeRedisValueInternal<T>((byte[]) obj));
    ...
    /// <param name="value">增量值(默认=1)</param>
    /// <returns></returns>
    public CSRedisClientPipe<decimal> IncrBy(string key, decimal value) =>
        PipeCommand(key, (c, k) => c.Value.IncrByFloat(k, value));
    ...
    public CSRedisClientPipe<string[]> MGet(params string[] keys)
    {
        if (keys == null || keys.Length == 0)
            throw new Exception("keys 不能为空");
        if (IsMultiNode)
            throw new Exception("MGet 管道命令，在分区模式下不可用");
        var prefix = Nodes.First().Value.Prefix;
    ...
    {
        var redisValule = rds.SerializeRedisValueInternal(value);
        if (expireSeconds <= 0 && exists == null)
            return PipeCommand(key, (c, k) =>
            {
                c.Value.Set(k, redisValule);
                return false;
            }, obj => obj?.ToString() == "OK");
        if (expireSeconds <= 0 && exists != null)
            return PipeCommand(key, (c, k) =>
            {
                c.Value.Set(k, redisValule, null, exists);
                return false;
            }, obj => obj?.ToString() == "OK");
        if (expireSeconds > 0 && exists == null)
            return PipeCommand(key, (c, k) =>
            {
                c.Value.Set(k, redisValule, expireSeconds);
                return false;
            }, obj => obj?.ToString() == "OK");
        if (expireSeconds > 0 && exists != null)
            return PipeCommand(key, (c, k) =>
            {
                c.Value.Set(k, redisValule, expireSeconds, exists);
                return false;
            }, obj => obj?.ToString() == "OK");
        return PipeCommand(key, (c, k) =>
        {
    ...
    public CSRedisClientPipe<bool> Set(string key, object value, TimeSpan expire, RedisExistence? exists = null)
    {
        ...
        if (expire > TimeSpan.Zero && exists == null)
            return PipeCommand(key, (c, k) =>
            {
                c.Value.Set(k, redisValule, expire);
                return false;
            }, obj => obj?.ToString() == "OK");
        if (expire > TimeSpan.Zero && exists != null)
            return PipeCommand(key, (c, k) =>
            {
                c.Value.Set(k, redisValule, expire, exists);
                return false;
            }, obj => obj?.ToString() == "OK");
        return PipeCommand(key, (c, k) =>
        {
    ...
    /// <param name="value">值</param>
    /// <returns></returns>
    public CSRedisClientPipe<bool> SetBit(string key, uint offset, bool value) =>
        PipeCommand(key, (c, k) => c.Value.SetBit(k, offset, value));
    ...
    /// <param name="value">值</param>
    /// <returns></returns>
    public CSRedisClientPipe<bool> SetNx(string key, object value) =>
        PipeCommand(key, (c, k) => c.Value.SetNx(k, rds.SerializeRedisValueInternal(value)));
    ...
    public CSRedisClientPipe<long> SetRange(string key, uint offset, object value) =>
        PipeCommand(key, (c, k) => c.Value.SetRange(k, offset, rds.SerializeRedisValueInternal(value)));
    ...
    /// <param name="expire">过期时间</param>
    /// <returns></returns>
    public CSRedisClientPipe<bool> ExpireAt(string key, DateTime expire) =>
        PipeCommand(key, (c, k) => c.Value.ExpireAt(k, expire));
    ...
    /// <param name="key">不含prefix前辍</param>
    /// <returns></returns>
    public CSRedisClientPipe<long?> ObjectRefCount(string key) =>
        PipeCommand(key, (c, k) => c.Value.Object(RedisObjectSubCommand.RefCount, k));
    ...
    /// <param name="key">不含prefix前辍</param>
    /// <returns></returns>
    public CSRedisClientPipe<long?> ObjectIdleTime(string key) =>
        PipeCommand(key, (c, k) => c.Value.Object(RedisObjectSubCommand.IdleTime, k));
    ...
    /// <returns></returns>
    public CSRedisClientPipe<bool> PExpire(string key, int milliseconds) =>
        PipeCommand(key, (c, k) => c.Value.PExpire(k, milliseconds));
    ...
    /// <param name="expire">过期时间</param>
    /// <returns></returns>
    public CSRedisClientPipe<bool> PExpireAt(string key, DateTime expire) =>
        PipeCommand(key, (c, k) => c.Value.PExpireAt(k, expire));
    ...
    {
        if (IsMultiNode)
            throw new Exception("Rename 管道命令，在分区模式下不可用");
        var prefix = Nodes.First().Value.Prefix;
        return PipeCommand(key, (c, k) =>
        {
            c.Value.Rename(k, prefix + newKey);
            return false;
        }, obj => obj?.ToString() == "OK");
    }
    ...
    {
        if (IsMultiNode)
            throw new Exception("RenameNx 管道命令，在分区模式下不可用");
        var prefix = Nodes.First().Value.Prefix;
        return PipeCommand(key, (c, k) =>
        {
            c.Value.RenameNx(k, prefix + newKey);
            return false;
        }, obj => obj?.ToString() == "OK");
    }
    ...
    /// <returns></returns>
    public CSRedisClientPipe<bool> Restore(string key, byte[] serializedValue) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.Restore(k, 0, serializedValue);
            return false;
        }, obj => obj?.ToString() == "OK");
    ...
    /// <param name="serializedValue">序列化值</param>
    /// <returns></returns>
    public CSRedisClientPipe<bool> Restore(string key, long ttlMilliseconds, byte[] serializedValue) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.Restore(k, ttlMilliseconds, serializedValue);
            return false;
        }, obj => obj?.ToString() == "OK");
    ...
    /// <returns></returns>
    public CSRedisClientPipe<string[]> Sort(string key, long? count = null, long offset = 0, string by = null,
        RedisSortDir? dir = null, bool? isAlpha = null, params string[] get)
    {
        if (IsMultiNode)
            throw new Exception("Sort 管道命令，在分区模式下不可用");
    ...
    /// <param name="get">根据排序的结果来取出相应的键值</param>
    /// <returns></returns>
    public CSRedisClientPipe<long> SortAndStore(string key, string destination, long? count = null, long offset = 0,
        string by = null, RedisSortDir? dir = null, bool? isAlpha = null, params string[] get)
    {
        if (IsMultiNode)
            throw new Exception("SortAndStore 管道命令，在分区模式下不可用");
        return PipeCommand(key,
    ...
    /// <param name="key">不含prefix前辍</param>
    /// <returns></returns>
    public CSRedisClientPipe<KeyType> Type(string key) =>
        PipeCommand(key, (c, k) =>
        {
            c.Value.Type(k);
            return KeyType.None;
        }, obj => Enum.TryParse(obj?.ToString(), true, out KeyType tryenum) ? tryenum : KeyType.None);
    ...
    {
        if (IsMultiNode)
            throw new Exception("Scan 管道命令，在分区模式下不可用");
    ...
    {
        if (IsMultiNode)
            throw new Exception("Scan<T> 管道命令，在分区模式下不可用");
        return PipeCommand("Scan<T>", (c, k) =>
        {
            c.Value.ScanBytes(cursor, pattern, count);
            return default(RedisScan<T>);
        }, obj =>
        {
            var scan = (RedisScan<byte[]>) obj;
