...
public class RedisClientPool : ObjectPool<RedisClient>
{
    ...
    {
        _policy = new RedisClientPoolPolicy {_pool = this};
        _policy.Connected += (s, o) =>
        {
            var rc = s as RedisClient;
            try
            {
            ...
            if (_policy._database > 0)
                rc.Select(_policy._database);
    ...
    public void Return(Object<RedisClient> obj, Exception exception, bool isRecreate = false)
    {
        if (exception != null)
        {
            try
            {
                try
                {
                    if (!obj.Value.IsConnected)
                        obj.Value.Connect(_policy._connectTimeout);
                    obj.Value.Ping();
    ...
    internal bool CheckAvailable() => LiveCheckAvailable();
...
public class RedisClientPoolPolicy : IPolicy<RedisClient>
{
    ...
    internal int _port = 6379, _database, _tryit, _connectTimeout = 5000, _syncTimeout = 10000;
    internal string _ip = "127.0.0.1", _user = "", _password = "", _clientname = "";
    internal bool _ssl, _testCluster = true, _asyncPipeline;
    ...
    public string Name
    {
        ...
        set
        {
            throw new Exception("RedisClientPoolPolicy 不提供设置 Name 属性值。");
        }
    }
    ...
    {
        return
            $"{endpoint}{(string.IsNullOrEmpty(_user) ? "" : $"user={_user}")},password={_password},defaultDatabase={_database},poolsize={PoolSize}," +
            ...
            for (var a = 1; a < vs.Length; a += 2)
            {
                ...
                switch (kv[0])
                {
                    ...
                    case "defaultdatabase":
                        ...
                        break;
                    case "poolsize":
                        PoolSize = int.TryParse(kv.Length > 1 ? kv[1].Trim() : "0", out var poolsize) == false || poolsize <= 0
                            ? 50
                            : poolsize;
                        break;
                    case "ssl":
                    ...
                    case "connecttimeout":
                        _connectTimeout =
                            int.TryParse(kv.Length > 1 ? kv[1].Trim() : "5000", out var connectTimeout) == false ||
                            connectTimeout <= 0
                                ? 5000
                                : connectTimeout;
                        break;
                    case "synctimeout":
                        _syncTimeout =
                            int.TryParse(kv.Length > 1 ? kv[1].Trim() : "10000", out var syncTimeout) == false || syncTimeout <= 0
                                ? 10000
                                : syncTimeout;
                        break;
                    case "idletimeout":
                        IdleTimeout = TimeSpan.FromMilliseconds(
                            int.TryParse(kv.Length > 1 ? kv[1].Trim() : "0", out var idleTimeout) == false || idleTimeout <= 0
                                ? 0
                                : idleTimeout);
                        break;
                    case "testcluster":
    ...
    public bool OnCheckAvailable(Object<RedisClient> obj)
    {
        obj.ResetValue();
        if (!obj.Value.IsConnected)
            obj.Value.Connect(_connectTimeout);
    ...
    public RedisClient OnCreate()
    {
        ...
        client.Connected += (s, o) =>
        {
            ...
            if (!string.IsNullOrEmpty(_clientname))
                client.ClientSetName(_clientname);
        };
        return client;
    }
    ...
    {
        if (_pool.Encoding != obj.Value.Encoding)
            obj.Value.Encoding = _pool.Encoding;
