...
{
    readonly ConcurrentDictionary<string, AutoPipe> _autoPipe = new ConcurrentDictionary<string, AutoPipe>();
    ...
    async Task<AutoPipe> GetClientAsync(RedisClientPool pool)
    {
        ...
        if (_autoPipe.TryGetValue(pool.Key, out var ap) && ap.IsSingleEndPipe == false)
        {
            if (pool.UnavailableException != null)
                throw new Exception($"【{pool._policy.Name}】状态不可用，等待后台检查程序恢复方可使用。{pool.UnavailableException?.Message}",
                    pool.UnavailableException);
                ...
                while (true)
                {
                    ...
                    if (rc._asyncPipe?.IsEmpty == false)
                    {
                        try
                        {
                            var ret = rc.EndPipe();
                            if (ret.Length == 1)
                                ap.IsSingleEndPipe = true;
                            else if (ret.Length > 1)
                                ap.IsSingleEndPipe = false;
        ...
        int errtimes = 0)
    {
        ...
        if (redirect == null)
            return await GetAndExecuteAsync(pool, handerAsync, jump - 1, errtimes);

        var redirectHanderAsync = redirect.Value.isMoved
            ? handerAsync
            : async redirectObj =>
            {
        ...
        return await GetAndExecuteAsync(GetRedirectPool(redirect.Value, pool), redirectHanderAsync, jump - 1);
    }

    async Task<T> NodesNotSupportAsync<T>(string[] keys, T defaultValue,
        Func<Object<RedisClient>, string[], Task<T>> callbackAsync)
    {
    ...
    Task<T> NodesNotSupportAsync<T>(string key, Func<Object<RedisClient>, string, Task<T>> callback)
    {
        ...
        return ExecuteScalarAsync(key, callback);
    }
    ...
    async public Task<T> CacheShellAsync<T>(string key, int timeoutSeconds, Func<Task<T>> getDataAsync)
    {
        ...
        await SetAsync(key, SerializeObject(ret), timeoutSeconds);
        return ret;
    }
    ...
    async public Task<T> CacheShellAsync<T>(string key, string field, int timeoutSeconds, Func<Task<T>> getDataAsync)
    {
        ...
        var ret = await getDataAsync();
        await HSetAsync(key, field, SerializeObject((ret, (long) DateTime.Now.Subtract(_dt1970).TotalSeconds)));
        return ret;
    }
    ...
    /// <returns></returns>
    async public Task<(string key, T value)[]> CacheShellAsync<T>(string key, string[] fields, int timeoutSeconds,
        Func<string[], Task<(string, T)[]>> getDataAsync)
    {
        fields = fields?.Distinct().ToArray();
        if (fields == null || fields.Length == 0)
            return new (string, T)[0];
        if (timeoutSeconds == 0)
            return await getDataAsync(fields);
    ...
    public partial class NodesServerManagerProvider
    {
        ...
        /// <returns></returns>
        public Task<(string node, string value)[]> ClientKillAsync(string ip, int port) =>
            NodesInternalAsync(c => c.Value.ClientKillAsync(ip, port));
        ...
        /// <returns></returns>
        public Task<(string node, long value)[]> ClientKillAsync(string addr = null, string id = null,
            ClientKillType? type = null, bool? skipMe = null) =>
            NodesInternalAsync(c => c.Value.ClientKillAsync(addr, id, type?.ToString(), skipMe));
        ...
        /// <returns></returns>
        public Task<(string node, string value)[]> ClientPauseAsync(TimeSpan timeout) =>
            NodesInternalAsync(c => c.Value.ClientPauseAsync(timeout));
        ...
        /// <returns></returns>
        public Task<(string node, string value)[]> ClientSetNameAsync(string connectionName) =>
            NodesInternalAsync(c => c.Value.ClientSetNameAsync(connectionName));
        ...
        /// <returns></returns>
        public Task<(string node, Dictionary<string, string> value)[]> ConfigGetAsync(string parameter) =>
            NodesInternalAsync(async c => (await c.Value.ConfigGetAsync(parameter)).ToDictionary(z => z.Item1, y => y.Item2));
        ...
        /// <returns></returns>
        public Task<(string node, string value)[]> ConfigSetAsync(string parameter, string value) =>
            NodesInternalAsync(c => c.Value.ConfigSetAsync(parameter, value));

        /// <summary>
        /// 重置 INFO 命令中的某些统计数据
        /// </summary>
        /// <returns></returns>
        public Task<(string node, string value)[]> ConfigResetStatAsync() =>
            NodesInternalAsync(c => c.Value.ConfigResetStatAsync());
        ...
        /// <returns></returns>
        public Task<(string node, string value)[]> InfoAsync(InfoSection? section = null) =>
            NodesInternalAsync(c => c.Value.InfoAsync(section?.ToString()));
        ...
        /// <returns></returns>
        public Task<(string node, string value)[]> ShutdownAsync(bool isSave = true) =>
            NodesInternalAsync(c => c.Value.ShutdownAsync(isSave));
        ...
        /// <param name="port">端口</param>
        /// <returns></returns>
        public Task<(string node, string value)[]> SlaveOfAsync(string host, int port) =>
            NodesInternalAsync(c => c.Value.SlaveOfAsync(host, port));
        ...
        /// <returns></returns>
        public Task<(string node, RedisSlowLogEntry[] value)[]> SlowLogGetAsync(long? count = null) =>
            NodesInternalAsync(c => c.Value.SlowLogGetAsync(count));
    ...
    public partial class NodeServerManagerProvider
    {
        ...
        public Task<string> ClientKillAsync(string ip, int port) =>
            _csredis.GetAndExecute(_pool, c => c.Value.ClientKillAsync(ip, port));
        ...
        /// <returns></returns>
        public Task<long>
            ClientKillAsync(string addr = null, string id = null, ClientKillType? type = null, bool? skipMe = null) =>
            _csredis.GetAndExecute(_pool, c => c.Value.ClientKillAsync(addr, id, type?.ToString(), skipMe));
        ...
        public Task<string> ClientPauseAsync(TimeSpan timeout) =>
            _csredis.GetAndExecute(_pool, c => c.Value.ClientPauseAsync(timeout));
        ...
        public Task<string> ClientSetNameAsync(string connectionName) =>
            _csredis.GetAndExecute(_pool, c => c.Value.ClientSetNameAsync(connectionName));
        ...
        /// <param name="parameter">参数</param>
        /// <returns></returns>
        async public Task<Dictionary<string, string>> ConfigGetAsync(string parameter) =>
            (await _csredis.GetAndExecute(_pool, c => c.Value.ConfigGetAsync(parameter)))
        ...
        public Task<string> ConfigSetAsync(string parameter, string value) =>
            _csredis.GetAndExecute(_pool, c => c.Value.ConfigSetAsync(parameter, value));
        ...
        /// <returns></returns>
        public Task<string> InfoAsync(InfoSection? section = null) =>
            _csredis.GetAndExecute(_pool, c => c.Value.InfoAsync(section?.ToString()));
        ...
        /// <param name="isSave">是否保存</param>
        /// <returns></returns>
        public Task<string> ShutdownAsync(bool isSave = true) =>
            _csredis.GetAndExecute(_pool, c => c.Value.ShutdownAsync(isSave));
        ...
        public Task<string> SlaveOfAsync(string host, int port) =>
            _csredis.GetAndExecute(_pool, c => c.Value.SlaveOfAsync(host, port));
        ...
        public Task<RedisSlowLogEntry[]> SlowLogGetAsync(long? count = null) =>
            _csredis.GetAndExecute(_pool, c => c.Value.SlowLogGetAsync(count));
    ...
    private Task<bool> AuthAsync(string nodeKey, string password) =>
        GetAndExecuteAsync(GetNodeOrThrowNotFound(nodeKey), async c => await c.Value.AuthAsync(password) == "OK");
    ...
    public Task<string> EchoAsync(string nodeKey, string message) =>
        GetAndExecuteAsync(GetNodeOrThrowNotFound(nodeKey), c => c.Value.EchoAsync(message));
    ...
    public Task<bool> PingAsync(string nodeKey) =>
        GetAndExecuteAsync(GetNodeOrThrowNotFound(nodeKey), async c => await c.Value.PingAsync() == "PONG");
    ...
    /// <returns></returns>
    [Obsolete("不建议手工执行，连接池自己管理最佳")]
    private Task<bool> QuitAsync(string nodeKey) =>
        GetAndExecuteAsync(GetNodeOrThrowNotFound(nodeKey), async c => await c.Value.QuitAsync() == "OK");
    ...
    private Task<bool> SelectAsync(string nodeKey, int index) =>
        GetAndExecuteAsync(GetNodeOrThrowNotFound(nodeKey), async c => await c.Value.SelectAsync(index) == "OK");
    ...
    async public Task<bool[]> ScriptExistsAsync(params string[] sha1)
    {
        ...
        foreach (var pool in Nodes.Values)
            ret.Add((await GetAndExecuteAsync(pool, c => c.Value.ScriptExistsAsync(sha1)))?.Where(z => z == false).Any() ==
                    false);
        return ret.ToArray();
    }
    ...
    {
        var msgid = await HIncrByAsync("csredisclient:Publish:msgid", channel);
    ...
    public Task<long> PublishNoneMessageIdAsync(string channel, string message) =>
        ExecuteScalarAsync(channel, (c, k) => c.Value.PublishAsync(channel, message));
    ...
    async public Task<Dictionary<string, long>> PubSubNumSubAsync(params string[] channels) =>
        (await ExecuteArrayAsync(channels, (c, k) =>
        {
            var prefix = (c.Pool as RedisClientPool).Prefix;
            return c.Value.PubSubNumSubAsync(k.Select(z =>
    ...
    /// <returns></returns>
    [Obsolete("分区模式下，若keys分散在多个分区节点时，将报错")]
    public Task<bool> PfMergeAsync(string destKey, params string[] sourceKeys) =>
        NodesNotSupportAsync(new[] {destKey}.Concat(sourceKeys).ToArray(), false,
            async (c, k) => await c.Value.PfMergeAsync(k.First(), k.Skip(1).ToArray()) == "OK");
    ...
    /// <param name="count">数量</param>
    /// <returns></returns>
    async public Task<(string member, decimal score)[]> ZPopMaxAsync(string key, long count) =>
        (await ExecuteScalarAsync(key, (c, k) => c.Value.ZPopMaxAsync(k, count))).Select(a => (a.Item1, a.Item2)).ToArray();
    ...
    /// <param name="count">数量</param>
    /// <returns></returns>
    async public Task<(T member, decimal score)[]> ZPopMaxAsync<T>(string key, long count) =>
        DeserializeRedisValueTuple1Internal<T, decimal>(await ExecuteScalarAsync(key,
            (c, k) => c.Value.ZPopMaxBytesAsync(k, count)));
    ...
    /// <param name="count">数量</param>
    /// <returns></returns>
    async public Task<(string member, decimal score)[]> ZPopMinAsync(string key, long count) =>
        (await ExecuteScalarAsync(key, (c, k) => c.Value.ZPopMinAsync(k, count))).Select(a => (a.Item1, a.Item2)).ToArray();
    ...
    /// <param name="count">数量</param>
    /// <returns></returns>
    async public Task<(T member, decimal score)[]> ZPopMinAsync<T>(string key, long count) =>
        DeserializeRedisValueTuple1Internal<T, decimal>(await ExecuteScalarAsync(key,
            (c, k) => c.Value.ZPopMinBytesAsync(k, count)));
    ...
    public Task<long> ZCountAsync(string key, decimal min, decimal max) =>
        ExecuteScalarAsync(key,
            (c, k) => c.Value.ZCountAsync(k, min == decimal.MinValue ? "-inf" : min.ToString(),
    ...
    public Task<long> ZCountAsync(string key, string min, string max) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.ZCountAsync(k, min, max));
    ...
    public Task<long> ZInterStoreAsync(string destination, decimal[] weights, RedisAggregate aggregate, params string[] keys)
    {
        ...
        return NodesNotSupportAsync(new[] {destination}.Concat(keys).ToArray(), 0,
            (c, k) => c.Value.ZInterStoreAsync(k.First(), weights, aggregate, k.Skip(1).ToArray()));
    }
    ...
    public Task<string[]> ZRangeAsync(string key, long start, long stop) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.ZRangeAsync(k, start, stop));
    ...
    /// <param name="stop">结束位置，0表示第一个元素，-1表示最后一个元素</param>
    /// <returns></returns>
    async public Task<T[]> ZRangeAsync<T>(string key, long start, long stop) =>
        DeserializeRedisValueArrayInternal<T>(await ExecuteScalarAsync(key, (c, k) => c.Value.ZRangeBytesAsync(k, start, stop)));
    ...
    /// <param name="stop">结束位置，0表示第一个元素，-1表示最后一个元素</param>
    /// <returns></returns>
    async public Task<(string member, decimal score)[]> ZRangeWithScoresAsync(string key, long start, long stop) =>
        (await ExecuteScalarAsync(key, (c, k) => c.Value.ZRangeWithScoresAsync(k, start, stop))).Select(a => (a.Item1, a.Item2))
        .ToArray();
    ...
    /// <param name="stop">结束位置，0表示第一个元素，-1表示最后一个元素</param>
    /// <returns></returns>
    async public Task<(T member, decimal score)[]> ZRangeWithScoresAsync<T>(string key, long start, long stop) =>
        DeserializeRedisValueTuple1Internal<T, decimal>(await ExecuteScalarAsync(key,
            (c, k) => c.Value.ZRangeBytesWithScoresAsync(k, start, stop)));
    ...
    public Task<string[]> ZRangeByScoreAsync(string key, decimal min, decimal max, long? count = null, long offset = 0) =>
        ExecuteScalarAsync(key,
            (c, k) => c.Value.ZRangeByScoreAsync(k, min == decimal.MinValue ? "-inf" : min.ToString(),
    ...
    async public Task<T[]> ZRangeByScoreAsync<T>(string key, decimal min, decimal max, long? count = null, long offset = 0) =>
        DeserializeRedisValueArrayInternal<T>(await ExecuteScalarAsync(key,
            (c, k) => c.Value.ZRangeBytesByScoreAsync(k, min == decimal.MinValue ? "-inf" : min.ToString(),
                max == decimal.MaxValue ? "+inf" : max.ToString(), false, offset, count)));
    ...
    async public Task<T[]> ZRangeByScoreAsync<T>(string key, string min, string max, long? count = null, long offset = 0) =>
        DeserializeRedisValueArrayInternal<T>(await ExecuteScalarAsync(key,
            (c, k) => c.Value.ZRangeBytesByScoreAsync(k, min, max, false, offset, count)));
    ...
    /// <param name="offset">返回条件偏移位置</param>
    /// <returns></returns>
    async public Task<(string member, decimal score)[]>
        ZRangeByScoreWithScoresAsync(string key, decimal min, decimal max, long? count = null, long offset = 0) =>
        (await ExecuteScalarAsync(key,
            (c, k) => c.Value.ZRangeByScoreWithScoresAsync(k, min == decimal.MinValue ? "-inf" : min.ToString(),
    ...
    /// <param name="offset">返回条件偏移位置</param>
    /// <returns></returns>
    async public Task<(T member, decimal score)[]>
        ZRangeByScoreWithScoresAsync<T>(string key, decimal min, decimal max, long? count = null, long offset = 0) =>
        DeserializeRedisValueTuple1Internal<T, decimal>(await ExecuteScalarAsync(key,
            (c, k) => c.Value.ZRangeBytesByScoreWithScoresAsync(k, min == decimal.MinValue ? "-inf" : min.ToString(),
    ...
    /// <param name="offset">返回条件偏移位置</param>
    /// <returns></returns>
    async public Task<(string member, decimal score)[]> ZRangeByScoreWithScoresAsync(string key, string min, string max,
        long? count = null, long offset = 0) =>
        (await ExecuteScalarAsync(key, (c, k) => c.Value.ZRangeByScoreWithScoresAsync(k, min, max, offset, count)))
    ...
    /// <returns></returns>
    async public Task<(T member, decimal score)[]>
        ZRangeByScoreWithScoresAsync<T>(string key, string min, string max, long? count = null, long offset = 0) =>
        DeserializeRedisValueTuple1Internal<T, decimal>(await ExecuteScalarAsync(key,
            (c, k) => c.Value.ZRangeBytesByScoreWithScoresAsync(k, min, max, offset, count)));
    ...
    public Task<long> ZRemRangeByRankAsync(string key, long start, long stop) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.ZRemRangeByRankAsync(k, start, stop));
    ...
    public Task<long> ZRemRangeByScoreAsync(string key, decimal min, decimal max) =>
        ExecuteScalarAsync(key,
            (c, k) => c.Value.ZRemRangeByScoreAsync(k, min == decimal.MinValue ? "-inf" : min.ToString(),
                max == decimal.MaxValue ? "+inf" : max.ToString()));
    ...
    public Task<long> ZRemRangeByScoreAsync(string key, string min, string max) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.ZRemRangeByScoreAsync(k, min, max));
    ...
    public Task<string[]> ZRevRangeAsync(string key, long start, long stop) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.ZRevRangeAsync(k, start, stop));
    ...
    /// <param name="stop">结束位置，0表示第一个元素，-1表示最后一个元素</param>
    /// <returns></returns>
    async public Task<T[]> ZRevRangeAsync<T>(string key, long start, long stop) =>
        DeserializeRedisValueArrayInternal<T>(
            await ExecuteScalarAsync(key, (c, k) => c.Value.ZRevRangeBytesAsync(k, start, stop)));
    ...
    /// <param name="stop">结束位置，0表示第一个元素，-1表示最后一个元素</param>
    /// <returns></returns>
    async public Task<(string member, decimal score)[]> ZRevRangeWithScoresAsync(string key, long start, long stop) =>
        (await ExecuteScalarAsync(key, (c, k) => c.Value.ZRevRangeWithScoresAsync(k, start, stop)))
    ...
    /// <param name="stop">结束位置，0表示第一个元素，-1表示最后一个元素</param>
    /// <returns></returns>
    async public Task<(T member, decimal score)[]> ZRevRangeWithScoresAsync<T>(string key, long start, long stop) =>
        DeserializeRedisValueTuple1Internal<T, decimal>(await ExecuteScalarAsync(key,
            (c, k) => c.Value.ZRevRangeBytesWithScoresAsync(k, start, stop)));
    ...
    /// <param name="offset">返回条件偏移位置</param>
    /// <returns></returns>
    public Task<string[]> ZRevRangeByScoreAsync(string key, decimal max, decimal min, long? count = null, long? offset = 0) =>
        ExecuteScalarAsync(key,
            (c, k) => c.Value.ZRevRangeByScoreAsync(k, max == decimal.MaxValue ? "+inf" : max.ToString(),
    ...
    async public Task<T[]> ZRevRangeByScoreAsync<T>(string key, decimal max, decimal min, long? count = null, long offset = 0) =>
        DeserializeRedisValueArrayInternal<T>(await ExecuteScalarAsync(key,
            (c, k) => c.Value.ZRevRangeBytesByScoreAsync(k, max == decimal.MaxValue ? "+inf" : max.ToString(),
                min == decimal.MinValue ? "-inf" : min.ToString(), false, offset, count)));
    ...
    /// <param name="offset">返回条件偏移位置</param>
    /// <returns></returns>
    public Task<string[]> ZRevRangeByScoreAsync(string key, string max, string min, long? count = null, long? offset = 0) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.ZRevRangeByScoreAsync(k, max, min, false, offset, count));
    ...
    async public Task<T[]> ZRevRangeByScoreAsync<T>(string key, string max, string min, long? count = null, long offset = 0) =>
        DeserializeRedisValueArrayInternal<T>(await ExecuteScalarAsync(key,
            (c, k) => c.Value.ZRevRangeBytesByScoreAsync(k, max, min, false, offset, count)));
    ...
    /// <returns></returns>
    async public Task<(string member, decimal score)[]>
        ZRevRangeByScoreWithScoresAsync(string key, decimal max, decimal min, long? count = null, long offset = 0) =>
        (await ExecuteScalarAsync(key,
            (c, k) => c.Value.ZRevRangeByScoreWithScoresAsync(k, max == decimal.MaxValue ? "+inf" : max.ToString(),
    ...
    /// <returns></returns>
    async public Task<(T member, decimal score)[]>
        ZRevRangeByScoreWithScoresAsync<T>(string key, decimal max, decimal min, long? count = null, long offset = 0) =>
        DeserializeRedisValueTuple1Internal<T, decimal>(await ExecuteScalarAsync(key,
            (c, k) => c.Value.ZRevRangeBytesByScoreWithScoresAsync(k, max == decimal.MaxValue ? "+inf" : max.ToString(),
    ...
    /// <param name="offset">返回条件偏移位置</param>
    /// <returns></returns>
    async public Task<(string member, decimal score)[]> ZRevRangeByScoreWithScoresAsync(string key, string max, string min,
        long? count = null, long offset = 0) =>
        (await ExecuteScalarAsync(key, (c, k) => c.Value.ZRevRangeByScoreWithScoresAsync(k, max, min, offset, count)))
        .Select(z => (z.Item1, z.Item2)).ToArray();
    ...
    /// <returns></returns>
    async public Task<(T member, decimal score)[]>
        ZRevRangeByScoreWithScoresAsync<T>(string key, string max, string min, long? count = null, long offset = 0) =>
        DeserializeRedisValueTuple1Internal<T, decimal>(await ExecuteScalarAsync(key,
            (c, k) => c.Value.ZRevRangeBytesByScoreWithScoresAsync(k, max, min, offset, count)));
    ...
    public Task<long> ZUnionStoreAsync(string destination, decimal[] weights, RedisAggregate aggregate, params string[] keys)
    {
        ...
        if (weights != null && weights.Length != keys.Length)
            throw new Exception("weights 和 keys 参数长度必须相同");
        return NodesNotSupportAsync(new[] {destination}.Concat(keys).ToArray(), 0,
            (c, k) => c.Value.ZUnionStoreAsync(k.First(), weights, aggregate, k.Skip(1).ToArray()));
    }
    ...
    /// <param name="count">数量</param>
    /// <returns></returns>
    async public Task<RedisScan<(string member, decimal score)>> ZScanAsync(string key, long cursor, string pattern = null,
        long? count = null)
    {
    ...
    /// <param name="count">数量</param>
    /// <returns></returns>
    async public Task<RedisScan<(T member, decimal score)>> ZScanAsync<T>(string key, long cursor, string pattern = null,
        long? count = null)
    {
    ...
    async public Task<T[]> ZRangeByLexAsync<T>(string key, string min, string max, long? count = null, long offset = 0) =>
        DeserializeRedisValueArrayInternal<T>(await ExecuteScalarAsync(key,
            (c, k) => c.Value.ZRangeBytesByLexAsync(k, min, max, offset, count)));
    ...
    public Task<string[]> SDiffAsync(params string[] keys) =>
        NodesNotSupportAsync(keys, new string[0], (c, k) => c.Value.SDiffAsync(k));
    ...
    /// <param name="keys">不含prefix前辍</param>
    /// <returns></returns>
    async public Task<T[]> SDiffAsync<T>(params string[] keys) =>
        DeserializeRedisValueArrayInternal<T>(await NodesNotSupportAsync(keys, new byte[0][],
            (c, k) => c.Value.SDiffBytesAsync(k)));
    ...
    public Task<long> SDiffStoreAsync(string destination, params string[] keys) =>
        NodesNotSupportAsync(new[] {destination}.Concat(keys).ToArray(), 0,
            (c, k) => c.Value.SDiffStoreAsync(k.First(), k.Skip(1).ToArray()));
    ...
    public Task<string[]> SInterAsync(params string[] keys) =>
        NodesNotSupportAsync(keys, new string[0], (c, k) => c.Value.SInterAsync(k));
    ...
    /// <param name="keys">不含prefix前辍</param>
    /// <returns></returns>
    async public Task<T[]> SInterAsync<T>(params string[] keys) =>
        DeserializeRedisValueArrayInternal<T>(await NodesNotSupportAsync(keys, new byte[0][],
            (c, k) => c.Value.SInterBytesAsync(k)));
    ...
    public Task<long> SInterStoreAsync(string destination, params string[] keys) =>
        NodesNotSupportAsync(new[] {destination}.Concat(keys).ToArray(), 0,
            (c, k) => c.Value.SInterStoreAsync(k.First(), k.Skip(1).ToArray()));
    ...
    /// <param name="key">不含prefix前辍</param>
    /// <returns></returns>
    async public Task<T[]> SMembersAsync<T>(string key) =>
        DeserializeRedisValueArrayInternal<T>(await ExecuteScalarAsync(key, (c, k) => c.Value.SMembersBytesAsync(k)));
    ...
    async public Task<bool> SMoveAsync(string source, string destination, object member)
    {
        ...
        if (Nodes.Count > 1)
        {
            ...
            if (rule1 != rule2)
            {
                if (await SRemAsync(source, member) <= 0)
                    return false;
    ...
    /// <param name="key">不含prefix前辍</param>
    /// <returns></returns>
    async public Task<T> SPopAsync<T>(string key) =>
        DeserializeRedisValueInternal<T>(await ExecuteScalarAsync(key, (c, k) => c.Value.SPopBytesAsync(k)));
    ...
    /// <param name="count">移除并返回的个数</param>
    /// <returns></returns>
    async public Task<T[]> SPopAsync<T>(string key, long count) =>
        DeserializeRedisValueArrayInternal<T>(await ExecuteScalarAsync(key, (c, k) => c.Value.SPopBytesAsync(k, count)));
    ...
    /// <param name="key">不含prefix前辍</param>
    /// <returns></returns>
    async public Task<T> SRandMemberAsync<T>(string key) =>
        DeserializeRedisValueInternal<T>(await ExecuteScalarAsync(key, (c, k) => c.Value.SRandMemberBytesAsync(k)));
    ...
    public Task<string[]> SRandMembersAsync(string key, int count = 1) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.SRandMembersAsync(k, count));
    ...
    /// <param name="count">返回个数</param>
    /// <returns></returns>
    async public Task<T[]> SRandMembersAsync<T>(string key, int count = 1) =>
        DeserializeRedisValueArrayInternal<T>(await ExecuteScalarAsync(key, (c, k) => c.Value.SRandMembersBytesAsync(k, count)));
    ...
    public Task<string[]> SUnionAsync(params string[] keys) =>
        NodesNotSupportAsync(keys, new string[0], (c, k) => c.Value.SUnionAsync(k));
    ...
    /// <param name="keys">不含prefix前辍</param>
    /// <returns></returns>
    async public Task<T[]> SUnionAsync<T>(params string[] keys) =>
        DeserializeRedisValueArrayInternal<T>(await NodesNotSupportAsync(keys, new byte[0][],
            (c, k) => c.Value.SUnionBytesAsync(k)));
    ...
    public Task<long> SUnionStoreAsync(string destination, params string[] keys) =>
        NodesNotSupportAsync(new[] {destination}.Concat(keys).ToArray(), 0,
            (c, k) => c.Value.SUnionStoreAsync(k.First(), k.Skip(1).ToArray()));
    ...
    public Task<RedisScan<string>> SScanAsync(string key, long cursor, string pattern = null, long? count = null) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.SScanAsync(k, cursor, pattern, count));
    ...
    /// <param name="index">索引</param>
    /// <returns></returns>
    async public Task<T> LIndexAsync<T>(string key, long index) =>
        DeserializeRedisValueInternal<T>(await ExecuteScalarAsync(key, (c, k) => c.Value.LIndexBytesAsync(k, index)));
    ...
    /// <param name="key">不含prefix前辍</param>
    /// <returns></returns>
    async public Task<T> LPopAsync<T>(string key) =>
        DeserializeRedisValueInternal<T>(await ExecuteScalarAsync(key, (c, k) => c.Value.LPopBytesAsync(k)));
    ...
    public Task<string[]> LRangeAsync(string key, long start, long stop) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.LRangeAsync(k, start, stop));
    ...
    /// <param name="stop">结束位置，0表示第一个元素，-1表示最后一个元素</param>
    /// <returns></returns>
    async public Task<T[]> LRangeAsync<T>(string key, long start, long stop) =>
        DeserializeRedisValueArrayInternal<T>(await ExecuteScalarAsync(key, (c, k) => c.Value.LRangeBytesAsync(k, start, stop)));
    ...
    public Task<bool> LTrimAsync(string key, long start, long stop) =>
        ExecuteScalarAsync(key, async (c, k) => await c.Value.LTrimAsync(k, start, stop) == "OK");
    ...
    /// <param name="key">不含prefix前辍</param>
    /// <returns></returns>
    async public Task<T> RPopAsync<T>(string key) =>
        DeserializeRedisValueInternal<T>(await ExecuteScalarAsync(key, (c, k) => c.Value.RPopBytesAsync(k)));
    ...
    public Task<string> RPopLPushAsync(string source, string destination) =>
        NodesNotSupportAsync(new[] {source, destination}, null, (c, k) => c.Value.RPopLPushAsync(k.First(), k.Last()));
    ...
    /// <param name="destination">目标key，不含prefix前辍</param>
    /// <returns></returns>
    async public Task<T> RPopLPushAsync<T>(string source, string destination) =>
        DeserializeRedisValueInternal<T>(await NodesNotSupportAsync(new[] {source, destination}, null,
            (c, k) => c.Value.RPopBytesLPushAsync(k.First(), k.Last())));
    ...
    /// <param name="fields">字段</param>
    /// <returns></returns>
    async public Task<long> HDelAsync(string key, params string[] fields) =>
        fields == null || fields.Any() == false ? 0 : await ExecuteScalarAsync(key, (c, k) => c.Value.HDelAsync(k, fields));
    ...
    /// <param name="field">字段</param>
    /// <returns></returns>
    async public Task<T> HGetAsync<T>(string key, string field) =>
        DeserializeRedisValueInternal<T>(await ExecuteScalarAsync(key, (c, k) => c.Value.HGetBytesAsync(k, field)));
    ...
    public Task<Dictionary<string, string>> HGetAllAsync(string key) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.HGetAllAsync(k));
    ...
    /// <param name="key">不含prefix前辍</param>
    /// <returns></returns>
    async public Task<Dictionary<string, T>> HGetAllAsync<T>(string key) =>
        DeserializeRedisValueDictionaryInternal<string, T>(await ExecuteScalarAsync(key, (c, k) => c.Value.HGetAllBytesAsync(k)));
    ...
    public Task<long> HIncrByAsync(string key, string field, long value = 1) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.HIncrByAsync(k, field, value));
    ...
    /// <param name="value">增量值(默认=1)</param>
    /// <returns></returns>
    public Task<decimal> HIncrByFloatAsync(string key, string field, decimal value) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.HIncrByFloatAsync(k, field, value));
    ...
    /// <param name="fields">字段</param>
    /// <returns></returns>
    async public Task<string[]> HMGetAsync(string key, params string[] fields) =>
    ...
    /// <returns></returns>
    async public Task<T[]> HMGetAsync<T>(string key, params string[] fields) =>
        fields == null || fields.Any() == false
            ? new T[0]
            : DeserializeRedisValueArrayInternal<T>(await ExecuteScalarAsync(key, (c, k) => c.Value.HMGetBytesAsync(k, fields)));
    ...
    /// <param name="key">不含prefix前辍</param>
    /// <returns></returns>
    async public Task<T[]> HValsAsync<T>(string key) =>
        DeserializeRedisValueArrayInternal<T>(await ExecuteScalarAsync(key, (c, k) => c.Value.HValsBytesAsync(k)));
    ...
    /// <param name="count">数量</param>
    /// <returns></returns>
    async public Task<RedisScan<(string field, string value)>> HScanAsync(string key, long cursor, string pattern = null,
        long? count = null)
    {
    ...
    /// <param name="count">数量</param>
    /// <returns></returns>
    async public Task<RedisScan<(string field, T value)>> HScanAsync<T>(string key, long cursor, string pattern = null,
        long? count = null)
    {
    ...
    public Task<long> BitCountAsync(string key, long start, long end) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.BitCountAsync(k, start, end));
    ...
    async public Task<long> BitOpAsync(RedisBitOp op, string destKey, params string[] keys)
    {
        ...
        return await NodesNotSupportAsync(new[] {destKey}.Concat(keys).ToArray(), 0,
            (c, k) => c.Value.BitOpAsync(op, k.First(), k.Skip(1).ToArray()));
    }
    ...
    public Task<long> BitPosAsync(string key, bool bit, long? start = null, long? end = null) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.BitPosAsync(k, bit, start, end));
    ...
    async public Task<T> GetAsync<T>(string key) =>
        DeserializeRedisValueInternal<T>(await ExecuteScalarAsync(key, (c, k) => c.Value.GetBytesAsync(k)));
    ...
    public Task<string> GetRangeAsync(string key, long start, long end) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.GetRangeAsync(k, start, end));
    ...
    /// <param name="end">结束位置，0表示第一个元素，-1表示最后一个元素</param>
    /// <returns></returns>
    async public Task<T> GetRangeAsync<T>(string key, long start, long end) =>
        DeserializeRedisValueInternal<T>(await ExecuteScalarAsync(key, (c, k) => c.Value.GetRangeBytesAsync(k, start, end)));
    ...
    {
        var args = SerializeRedisValueInternal(value);
        return DeserializeRedisValueInternal<T>(await ExecuteScalarAsync(key, (c, k) => c.Value.GetSetBytesAsync(k, args)));
    }
    ...
    /// <param name="value">增量值(默认=1)</param>
    /// <returns></returns>
    public Task<decimal> IncrByFloatAsync(string key, decimal value) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.IncrByFloatAsync(k, value));
    ...
    /// <param name="keys">不含prefix前辍</param>
    /// <returns></returns>
    async public Task<T[]> MGetAsync<T>(params string[] keys) =>
        DeserializeRedisValueArrayInternal<T>(await ExecuteArrayAsync(keys, (c, k) => c.Value.MGetBytesAsync(k)));
    ...
    async internal Task<bool> MSetInternalAsync(RedisExistence exists, params object[] keyValues)
    {
        ...
        Func<Object<RedisClient>, string[], Task<long>> handle = async (c, k) =>
        {
            ...
            if (exists == RedisExistence.Nx)
                return await c.Value.MSetNxAsync(parms) ? 1 : 0;
            return await c.Value.MSetAsync(parms) == "OK" ? 1 : 0;
        };
        if (exists == RedisExistence.Nx)
            return await NodesNotSupportAsync(dic.Keys.ToArray(), 0, handle) > 0;
    ...
    async public Task<bool> SetAsync(string key, object value, int expireSeconds = -1, RedisExistence? exists = null)
    {
        ...
        if (expireSeconds <= 0 && exists == null)
            return await ExecuteScalarAsync(key, (c, k) => c.Value.SetAsync(k, redisValule)) == "OK";
        if (expireSeconds <= 0 && exists != null)
            return await ExecuteScalarAsync(key, (c, k) => c.Value.SetAsync(k, redisValule, null, exists)) == "OK";
        if (expireSeconds > 0 && exists == null)
            return await ExecuteScalarAsync(key, (c, k) => c.Value.SetAsync(k, redisValule, expireSeconds)) == "OK";
        if (expireSeconds > 0 && exists != null)
            return await ExecuteScalarAsync(key, (c, k) => c.Value.SetAsync(k, redisValule, expireSeconds, exists)) == "OK";
        return false;
    }
    ...
    async public Task<bool> SetAsync(string key, object value, TimeSpan expire, RedisExistence? exists = null)
    {
        ...
        if (expire <= TimeSpan.Zero && exists == null)
            return await ExecuteScalar(key, (c, k) => c.Value.SetAsync(k, redisValule)) == "OK";
        if (expire <= TimeSpan.Zero && exists != null)
            return await ExecuteScalar(key, (c, k) => c.Value.SetAsync(k, redisValule, null, exists)) == "OK";
        if (expire > TimeSpan.Zero && exists == null)
            return await ExecuteScalar(key, (c, k) => c.Value.SetAsync(k, redisValule, expire)) == "OK";
        if (expire > TimeSpan.Zero && exists != null)
            return await ExecuteScalar(key, (c, k) => c.Value.SetAsync(k, redisValule, expire, exists)) == "OK";
        return false;
    }
    ...
    public Task<bool> SetBitAsync(string key, uint offset, bool value) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.SetBitAsync(k, offset, value));
    ...
    public Task<bool> ExpireAsync(string key, TimeSpan expire) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.ExpireAsync(k, expire));
    ...
    public Task<bool> ExpireAtAsync(string key, DateTime expire) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.ExpireAtAsync(k, expire));
    ...
    public Task<long?> ObjectRefCountAsync(string key) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.ObjectAsync(RedisObjectSubCommand.RefCount, k));
    ...
    public Task<long?> ObjectIdleTimeAsync(string key) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.ObjectAsync(RedisObjectSubCommand.IdleTime, k));
    ...
    public Task<bool> PExpireAsync(string key, int milliseconds) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.PExpireAsync(k, milliseconds));
    ...
    public Task<bool> PExpireAsync(string key, TimeSpan expire) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.PExpireAsync(k, expire));
    ...
    public Task<bool> PExpireAtAsync(string key, DateTime expire) =>
        ExecuteScalarAsync(key, (c, k) => c.Value.PExpireAtAsync(k, expire));
    ...
    public Task<string> RandomKeyAsync() =>
        GetAndExecuteAsync(Nodes[NodesIndex[_rnd.Next(0, NodesIndex.Count)]], async c =>
        {
    ...
    public Task<bool> RenameNxAsync(string key, string newKey) =>
        NodesNotSupportAsync(new[] {key, newKey}, false, (c, k) => c.Value.RenameNxAsync(k.First(), k.Last()));
    ...
    public Task<bool> RestoreAsync(string key, byte[] serializedValue) =>
        ExecuteScalarAsync(key, async (c, k) => await c.Value.RestoreAsync(k, 0, serializedValue) == "OK");
    ...
    public Task<bool> RestoreAsync(string key, long ttlMilliseconds, byte[] serializedValue) =>
        ExecuteScalarAsync(key, async (c, k) => await c.Value.RestoreAsync(k, ttlMilliseconds, serializedValue) == "OK");
    ...
    /// <returns></returns>
    public Task<string[]> SortAsync(string key, long? count = null, long offset = 0, string by = null, RedisSortDir? dir = null,
    ...
    /// <param name="get">根据排序的结果来取出相应的键值</param>
    /// <returns></returns>
    public Task<long> SortAndStoreAsync(string key, string destination, long? count = null, long offset = 0, string by = null,
        RedisSortDir? dir = null, bool? isAlpha = null, params string[] get) =>
        NodesNotSupportAsync(key,
            (c, k) => c.Value.SortAndStoreAsync(k, (c.Pool as RedisClientPool)?.Prefix + destination, offset, count, by, dir,
                isAlpha, get));
    ...
    /// <param name="key">不含prefix前辍</param>
    /// <returns></returns>
    async public Task<KeyType> TypeAsync(string key) =>
        Enum.TryParse(await ExecuteScalarAsync(key, (c, k) => c.Value.TypeAsync(k)), true, out KeyType tryenum)
            ? tryenum
            : KeyType.None;
    ...
    public Task<RedisScan<string>> ScanAsync(long cursor, string pattern = null, long? count = null) =>
        NodesNotSupportAsync("ScanAsync", (c, k) => c.Value.ScanAsync(cursor, pattern, count));
    ...
    async public Task<bool> GeoAddAsync(string key, decimal longitude, decimal latitude, object member) =>
        await GeoAddAsync(key, (longitude, latitude, member)) == 1;
    ...
    /// <returns></returns>
    async public Task<string[]> GeoRadiusAsync(string key, decimal longitude, decimal latitude, decimal radius,
        GeoUnit unit = GeoUnit.m, long? count = null, GeoOrderBy? sorting = null) =>
        (await ExecuteScalarAsync(key, (c, k) => c.Value.GeoRadiusAsync(k, longitude, latitude, radius, unit, count, sorting)))
    ...
    /// <param name="sorting">排序</param>
    /// <returns></returns>
    async public Task<T[]> GeoRadiusAsync<T>(string key, decimal longitude, decimal latitude, decimal radius,
        GeoUnit unit = GeoUnit.m, long? count = null, GeoOrderBy? sorting = null) =>
        (await ExecuteScalarAsync(key,
            (c, k) => c.Value.GeoRadiusBytesAsync(k, longitude, latitude, radius, unit, count, sorting)))
    ...
    /// <param name="sorting">排序</param>
    /// <returns></returns>
    async public Task<(string member, decimal dist)[]> GeoRadiusWithDistAsync(string key, decimal longitude, decimal latitude,
        decimal radius, GeoUnit unit = GeoUnit.m, long? count = null, GeoOrderBy? sorting = null) =>
        (await ExecuteScalarAsync(key,
            (c, k) => c.Value.GeoRadiusAsync(k, longitude, latitude, radius, unit, count, sorting, false, true)))
    ...
    /// <param name="sorting">排序</param>
    /// <returns></returns>
    async public Task<(T member, decimal dist)[]> GeoRadiusWithDistAsync<T>(string key, decimal longitude, decimal latitude,
        decimal radius, GeoUnit unit = GeoUnit.m, long? count = null, GeoOrderBy? sorting = null) =>
        (await ExecuteScalarAsync(key,
            (c, k) => c.Value.GeoRadiusBytesAsync(k, longitude, latitude, radius, unit, count, sorting, false, true)))
    ...
    /// <param name="sorting">排序</param>
    /// <returns></returns>
    async private Task<(string member, decimal longitude, decimal latitude)[]> GeoRadiusWithCoordAsync(string key,
        decimal longitude, decimal latitude, decimal radius, GeoUnit unit = GeoUnit.m, long? count = null,
        GeoOrderBy? sorting = null) =>
        (await ExecuteScalarAsync(key,
            (c, k) => c.Value.GeoRadiusAsync(k, longitude, latitude, radius, unit, count, sorting, true)))
    ...
    /// <param name="sorting">排序</param>
    /// <returns></returns>
    async private Task<(T member, decimal longitude, decimal latitude)[]> GeoRadiusWithCoordAsync<T>(string key,
        decimal longitude, decimal latitude, decimal radius, GeoUnit unit = GeoUnit.m, long? count = null,
        GeoOrderBy? sorting = null) =>
        (await ExecuteScalarAsync(key,
            (c, k) => c.Value.GeoRadiusBytesAsync(k, longitude, latitude, radius, unit, count, sorting, true)))
    ...
    /// <param name="sorting">排序</param>
    /// <returns></returns>
    async public Task<(string member, decimal dist, decimal longitude, decimal latitude)[]> GeoRadiusWithDistAndCoordAsync(
        string key, decimal longitude, decimal latitude, decimal radius, GeoUnit unit = GeoUnit.m, long? count = null,
        GeoOrderBy? sorting = null) =>
        (await ExecuteScalarAsync(key,
            (c, k) => c.Value.GeoRadiusAsync(k, longitude, latitude, radius, unit, count, sorting, true, true)))
    ...
    /// <param name="sorting">排序</param>
    /// <returns></returns>
    async public Task<(T member, decimal dist, decimal longitude, decimal latitude)[]> GeoRadiusWithDistAndCoordAsync<T>(
        string key, decimal longitude, decimal latitude, decimal radius, GeoUnit unit = GeoUnit.m, long? count = null,
        GeoOrderBy? sorting = null) =>
        (await ExecuteScalarAsync(key,
            (c, k) => c.Value.GeoRadiusBytesAsync(k, longitude, latitude, radius, unit, count, sorting, true, true)))
    ...
    /// <param name="sorting">排序</param>
    /// <returns></returns>
    async public Task<string[]> GeoRadiusByMemberAsync(string key, object member, decimal radius, GeoUnit unit = GeoUnit.m,
        long? count = null, GeoOrderBy? sorting = null) =>
        (await ExecuteScalarAsync(key, (c, k) => c.Value.GeoRadiusByMemberAsync(k, member, radius, unit, count, sorting)))
        .Select(a => a.member).ToArray();
    ...
    /// <param name="sorting">排序</param>
    /// <returns></returns>
    async public Task<T[]> GeoRadiusByMemberAsync<T>(string key, object member, decimal radius, GeoUnit unit = GeoUnit.m,
        long? count = null, GeoOrderBy? sorting = null) =>
        (await ExecuteScalarAsync(key, (c, k) => c.Value.GeoRadiusBytesByMemberAsync(k, member, radius, unit, count, sorting)))
        .Select(a => DeserializeRedisValueInternal<T>(a.member)).ToArray();
    ...
    /// <param name="sorting">排序</param>
    /// <returns></returns>
    async public Task<(string member, decimal dist)[]> GeoRadiusByMemberWithDistAsync(string key, object member, decimal radius,
        GeoUnit unit = GeoUnit.m, long? count = null, GeoOrderBy? sorting = null) =>
        (await ExecuteScalarAsync(key,
            (c, k) => c.Value.GeoRadiusByMemberAsync(k, member, radius, unit, count, sorting, false, true)))
        .Select(a => (a.member, a.dist)).ToArray();
    ...
    /// <param name="sorting">排序</param>
    /// <returns></returns>
    async public Task<(T member, decimal dist)[]> GeoRadiusByMemberWithDistAsync<T>(string key, object member, decimal radius,
        GeoUnit unit = GeoUnit.m, long? count = null, GeoOrderBy? sorting = null) =>
        (await ExecuteScalarAsync(key,
            (c, k) => c.Value.GeoRadiusBytesByMemberAsync(k, member, radius, unit, count, sorting, false, true)))
        .Select(a => (DeserializeRedisValueInternal<T>(a.member), a.dist)).ToArray();
    ...
    /// <param name="sorting">排序</param>
    /// <returns></returns>
    async private Task<(string member, decimal longitude, decimal latitude)[]> GeoRadiusByMemberWithCoordAsync(string key,
        object member, decimal radius, GeoUnit unit = GeoUnit.m, long? count = null, GeoOrderBy? sorting = null) =>
        (await ExecuteScalarAsync(key, (c, k) => c.Value.GeoRadiusByMemberAsync(k, member, radius, unit, count, sorting, true)))
        .Select(a => (a.member, a.longitude, a.latitude)).ToArray();
    ...
    /// <param name="sorting">排序</param>
    /// <returns></returns>
    async private Task<(T member, decimal longitude, decimal latitude)[]> GeoRadiusByMemberWithCoordAsync<T>(string key,
        object member, decimal radius, GeoUnit unit = GeoUnit.m, long? count = null, GeoOrderBy? sorting = null) =>
        (await ExecuteScalarAsync(key,
            (c, k) => c.Value.GeoRadiusBytesByMemberAsync(k, member, radius, unit, count, sorting, true)))
        .Select(a => (DeserializeRedisValueInternal<T>(a.member), a.longitude, a.latitude)).ToArray();
    ...
    /// <param name="sorting">排序</param>
    /// <returns></returns>
    async public Task<(string member, decimal dist, decimal longitude, decimal latitude)[]>
        GeoRadiusByMemberWithDistAndCoordAsync(string key, object member, decimal radius, GeoUnit unit = GeoUnit.m,
            long? count = null, GeoOrderBy? sorting = null) =>
        (await ExecuteScalarAsync(key,
            (c, k) => c.Value.GeoRadiusByMemberAsync(k, member, radius, unit, count, sorting, true, true)))
        .Select(a => (a.member, a.dist, a.longitude, a.latitude)).ToArray();
    ...
    /// <param name="sorting">排序</param>
    /// <returns></returns>
    async public Task<(T member, decimal dist, decimal longitude, decimal latitude)[]> GeoRadiusByMemberWithDistAndCoordAsync<T>(
        string key, object member, decimal radius, GeoUnit unit = GeoUnit.m, long? count = null, GeoOrderBy? sorting = null) =>
        (await ExecuteScalarAsync(key,
            (c, k) => c.Value.GeoRadiusBytesByMemberAsync(k, member, radius, unit, count, sorting, true, true)))
        .Select(a => (DeserializeRedisValueInternal<T>(a.member), a.dist, a.longitude, a.latitude)).ToArray();
