...
class RedisArray : RedisCommand<object[]>
{
    ...
    public RedisArray(string command, params object[] args) : base(command, args)
    {
    }
    ...
    public override object[] Parse(RedisReader reader)
    {
        ...
        var results = new object[_parsers.Count];
    ...
    {
        protected RedisCommand<T> MemberCommand { get; }

        public Generic(RedisCommand<T> memberCommand) : base(memberCommand.Command, memberCommand.Arguments)
        {
        ...
        protected virtual T[] Read(long count, RedisReader reader)
        {
            ...
            for (var i = 0; i < array.Length; i++)
                array[i] = MemberCommand.Parse(reader);
            return array;
        }
    }
    ...
    public class IndexOf<T> : RedisCommand<T>
    {
        ...
        public IndexOf(RedisCommand<T> command, int index) : base(command.Command, command.Arguments)
        {
            _command = command;
            _index = index;
        }
    ...
    {
        public Strings(string command, params object[] args) : base(new RedisString(command, args))
        {
        }
    }
    ...
    {
        public Bytes(string command, params object[] args) : base(new RedisBytes(command, args))
        {
        }
    }
    ...
    {
        public StrongPairs(RedisCommand<T1> command1, RedisCommand<T2> command2, string command, params object[] args) : base(
            new RedisTuple.Generic<T1, T2>.Repeating(command1, command2, command, args))
        {
        }
    ...
    {
        public WeakPairs(string command, params object[] args) : base(new RedisString.Converter<T1>(null),
            new RedisString.Converter<T2>(null), command, args)
        {
        }
    }
}