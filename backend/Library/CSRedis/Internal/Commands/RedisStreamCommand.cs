...
{
    public RedisXRangeCommand(string command, params object[] args) : base(command, args)
    {
    }
    ...
    public override (string id, string[] items)[] Parse(RedisReader reader)
    {
        ...
        for (var a = 0; a < count; a++)
        {
            ...
            if (lvl2Count != 2)
                throw new RedisProtocolException("XRange/XRevRange/XClaim 返回数据格式 2级 MultiBulk 长度应该为 2");
...
{
    public RedisXReadCommand(string command, params object[] args) : base(command, args)
    {
    }
    ...
    public override (string key, (string id, string[] items)[] data)[] Parse(RedisReader reader)
    {
        ...
        for (var a = 0; a < count; a++)
        {
            ...
            var lvl2Count = reader.ReadInt(false);
            if (lvl2Count != 2)
                throw new RedisProtocolException("XRead 返回数据格式 2级 MultiBulk 长度应该为 2");
            ...
            for (var c = 0; c < lvl3Count; c++)
            {
                ...
                if (lvl4Count != 2)
                    throw new RedisProtocolException("XRead 返回数据格式 4级 MultiBulk 长度应该为 2");
...
{
    public RedisXPendingCommand(string command, params object[] args) : base(command, args)
    {
    }
    ...
    public override (long count, string minId, string maxId, (string consumer, long count)[] pendings) Parse(RedisReader reader)
    {
        ...
        var count = reader.ReadInt(false);
        if (count != 4)
            throw new RedisProtocolException("XPedding 返回数据格式 1级 MultiBulk 长度应该为 4");
        ...
        for (var a = 0; a < lvl2Count; a++)
        {
            ...
            var lvl3Count = reader.ReadInt(false);
            if (lvl3Count != 2)
                throw new RedisProtocolException("XPedding 返回数据格式 3级 MultiBulk 长度应该为 2");
...
{
    public RedisXPendingStartEndCountCommand(string command, params object[] args) : base(command, args)
    {
    }
...
class RedisXInfoStreamCommand : RedisCommand<(long length, long radixTreeKeys, long radixTreeNodes, long groups, string
    lastGeneratedId, (string id, string[] items) firstEntry, (string id, string[] items) lastEntry)>
{
    public RedisXInfoStreamCommand(string command, params object[] args) : base(command, args)
    {
    }

    public override (long length, long radixTreeKeys, long radixTreeNodes, long groups, string lastGeneratedId, (string id, string
        [] items) firstEntry, (string id, string[] items) lastEntry) Parse(RedisReader reader)
    {
        reader.ExpectType(RedisMessage.MultiBulk);
        var count = reader.ReadInt(false);
        if (count % 2 != 0)
            throw new RedisProtocolException("XInfo Stream 返回数据格式 1级 MultiBulk 长度应该为偶数");

        var ret =
            default((long length, long radixTreeKeys, long radixTreeNodes, long groups, string lastGeneratedId, (string id, string
                [] items) firstEntry, (string id, string[] items) lastEntry));
        ...
        for (var a = 0; a < count; a += 2)
        {
            ...
            switch (key)
            {
                ...
                case "last-entry":
                    ...
                    var lvl2Count = reader.ReadInt(false);
                    if (lvl2Count != 2)
                        throw new RedisProtocolException("XInfo Stream 返回数据格式 2级 MultiBulk 长度应该为 2");
                    ...
                    if (key == "first-entry")
                        ret.firstEntry = (id, items);
                    else
                        ret.lastEntry = (id, items);
                    break;
                default:
...
{
    public RedisXInfoGroupsCommand(string command, params object[] args) : base(command, args)
    {
    }
    ...
    public override (string name, long consumers, long pending, string lastDeliveredId)[] Parse(RedisReader reader)
    {
        ...
        for (var a = 0; a < count; a++)
        {
            ...
            var lvl2Count = reader.ReadInt(false);
            if (lvl2Count % 2 != 0)
                throw new RedisProtocolException("XInfo Groups 返回数据格式 2级 MultiBulk 长度应该为偶数");
...
{
    public RedisXInfoConsumersCommand(string command, params object[] args) : base(command, args)
    {
    }
    ...
    public override (string name, long pending, long idle)[] Parse(RedisReader reader)
    {
        ...
        for (var a = 0; a < count; a++)
        {
            ...
            var lvl2Count = reader.ReadInt(false);
            if (lvl2Count % 2 != 0)
                throw new RedisProtocolException("XInfo Consumers 返回数据格式 2级 MultiBulk 长度应该为偶数");
