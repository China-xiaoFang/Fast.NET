...
public class ObjectPool<T> : IObjectPool<T>
{
    ...
    private readonly ConcurrentQueue<TaskCompletionSource<Object<T>>> _getAsyncQueue =
        new ConcurrentQueue<TaskCompletionSource<Object<T>>>();

    private readonly ConcurrentQueue<bool> _getQueue = new ConcurrentQueue<bool>();

    public bool IsAvailable => UnavailableException == null;
    ...
    private readonly object UnavailableLock = new object();
    ...
    private void CheckAvailable(int interval)
    {
        ...
        {
            if (UnavailableException != null)
                TestTrace.WriteLine($"【{Policy.Name}】Next recovery time：{DateTime.Now.AddSeconds(interval)}",
            ...
            while (UnavailableException != null)
            {
                ...
                try
                {
                    ...
                    if (conn == null)
                        throw new Exception($"CheckAvailable 无法获得资源，{Statistics}");
                ...
                {
                    TestTrace.WriteLine($"【{Policy.Name}】Next recovery time: {DateTime.Now.AddSeconds(interval)} ({ex.Message})",
                        ConsoleColor.DarkYellow);
                }
            }
    ...
    protected bool LiveCheckAvailable()
    {
        try
        {
            var conn = GetFree(false);
            if (conn == null)
                throw new Exception($"LiveCheckAvailable 无法获得资源，{Statistics}");
    ...
    public string Statistics =>
        $"Pool: {_freeObjects.Count}/{_allObjects.Count}, Get wait: {_getSyncQueue.Count}, GetAsync wait: {_getAsyncQueue.Count}";
    ...
    public string StatisticsFullily
    {
        get
        {
            ...
            {
                sb.AppendLine(
                    $"{obj.Value}, Times: {obj.GetTimes}, ThreadId(R/G): {obj.LastReturnThreadId}/{obj.LastGetThreadId}, Time(R/G): {obj.LastReturnTime.ToString("yyyy-MM-dd HH:mm:ss:ms")}/{obj.LastGetTime.ToString("yyyy-MM-dd HH:mm:ss:ms")}, ");
            }
    ...
    public ObjectPool(int poolsize, Func<T> createObject, Action<Object<T>> onGetObject = null) : this(
        new DefaultPolicy<T> {PoolSize = poolsize, CreateObject = createObject, OnGetObject = onGetObject})
    {
    }
    ...
    private Object<T> GetFree(bool checkAvailable)
    {
        ...
        if (obj != null && obj.Value == null || obj != null && Policy.IdleTimeout > TimeSpan.Zero &&
            DateTime.Now.Subtract(obj.LastReturnTime) > Policy.IdleTimeout)
        {
            try
            {
                obj.ResetValue();
        ...
        if (obj == null)
        {
            if (Policy.AsyncGetCapacity > 0 && _getAsyncQueue.Count >= Policy.AsyncGetCapacity - 1)
                throw new OutOfMemoryException(
                    $"ObjectPool.GetAsync 无可用资源且队列过长，Policy.AsyncGetCapacity = {Policy.AsyncGetCapacity}。");
