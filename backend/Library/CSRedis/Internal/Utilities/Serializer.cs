...
internal static class Serializer<T> where T : class
{
    ...
    static Func<T, Dictionary<string, string>> CompilePropertySerializer()
    {
        ...
        var d_setters = o_t.GetProperties(BindingFlags.Public | BindingFlags.Instance) // build setters via Add(k,v)
            .Where(x => x.CanRead).Select(x =>
            {
                var prop = Expression.Property(o, x.Name);
                var prop_mi_to_string = x.PropertyType.GetMethod("ToString", new Type[0]);
                var add_to_dict =
                    Expression.Call(d, d_add, Expression.Constant(x.Name), Expression.Call(prop, prop_mi_to_string));

                if (!x.PropertyType.IsByRef)
                    return add_to_dict;
                return (Expression) Expression.IfThen(Expression.Not(Expression.Equal(prop, Expression.Constant(null))),
                    add_to_dict);
            });
    ...
    static Func<Dictionary<string, string>, T> CompilePropertyDeserializer()
    {
        ...
        var binds = o_t.GetProperties(BindingFlags.Public | BindingFlags.Instance).Where(x => x.CanRead).Select(x =>
        {
            var value_t = x.PropertyType;
            var value = Expression.Variable(value_t, "value");
            var target = Expression.Label(x.PropertyType);

            return Expression.Bind(x,
                Expression.Block(new[] {item, value},
                    Expression.Assign(tc, Expression.Call(null, td_mi_get_converter, Expression.Constant(x.PropertyType))),
                    Expression.IfThen(Expression.Call(d, d_mi_try_get_value, Expression.Constant(x.Name), item),
                        Expression.IfThen(Expression.NotEqual(item, Expression.Constant(null)),
                            Expression.IfThen(Expression.Call(tc, tc_mi_can_convert_from, Expression.Constant(typeof(String))),
                                Expression.Block(
                                    Expression.Assign(value,
                                        Expression.Convert(Expression.Call(tc, tc_mi_convert_from, item), x.PropertyType)),
                                    Expression.Return(target, value, x.PropertyType))))), Expression.Label(target, value)));
        }).ToArray();

        var body = Expression.Block(new[] {o, tc}, Expression.MemberInit(o_new, binds));

        return Expression.Lambda<Func<Dictionary<string, string>, T>>(body, d).Compile();
    }
    ...
    static Func<T, Dictionary<string, string>> CompileSerializer()
    {
        ...
        var fc_init =
            Expression.MemberInit(Expression.New(typeof(FormatterConverter))); //Expression.MemberInit(Expression.New(fc_t));
        ...
        var body = Expression.Block(new[] {d, fc, info, ctx}, Expression.Assign(d, d_init), Expression.Assign(fc, fc_init),
            Expression.Assign(info, info_init), Expression.Assign(ctx, ctx_init),
            Expression.Call(o, o_get_object_data, info, ctx), Expression.Block(new[] {enumerator},
                Expression.Assign(enumerator, Expression.Call(info, info_get_enumerator)), Expression.Loop(Expression.IfThenElse(
                        Expression.Call(enumerator, enumerator_move_next), // test
                        Expression.IfThen(Expression.NotEqual(enumerator_value, Expression.Constant(null)),
    ...
    static Func<Dictionary<string, string>, T> CompileDeserializer()
    {
        ...
        var body = Expression.Block(new[] {fc, info, ctx, enumerator}, Expression.Assign(fc, fc_init),
            Expression.Assign(info, info_init), Expression.Assign(ctx, ctx_init),
            Expression.Assign(enumerator, Expression.Call(d, d_mi_get_enumerator)),
            Expression.Loop(
                Expression.IfThenElse(Expression.Call(enumerator, enumerator_mi_move_next),
                    Expression.Call(info, info_mi_add_value, Expression.Property(enumerator_current, kvp_pi_key),
                        Expression.Property(enumerator_current, kvp_pi_value)), Expression.Break(exit_loop)), exit_loop),
