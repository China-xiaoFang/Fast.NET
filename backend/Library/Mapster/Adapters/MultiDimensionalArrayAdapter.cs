...
public class MultiDimensionalArrayAdapter : BaseAdapter
{
    ...
    {
        return arg.SourceType.IsCollection() && arg.DestinationType.IsArray && arg.DestinationType.GetArrayRank() > 1;
    }
    ...
    protected override Expression TransformSource(Expression source)
    {
        ...
        {
            var cast = typeof(Enumerable).GetMethod(nameof(Enumerable.Cast))!.MakeGenericMethod(elemType);
            transformed = Expression.Call(cast, transformed);
        }

        var toList = typeof(Enumerable).GetMethod(nameof(Enumerable.ToList))!.MakeGenericMethod(elemType);
    ...
    private static IEnumerable<Expression> GetArrayBounds(Expression source, Type destinationType)
    {
        ...
        else
        {
            ...
            for (var i = srcRank; i < destRank; i++)
            {
                yield return Expression.Constant(1);
            }

            for (var i = 0; i < srcRank; i++)
            {
    ...
    {
        if (source.Type.IsArray && source.Type.GetArrayRank() == destination.Type.GetArrayRank() &&
            source.Type.GetElementType() == destination.Type.GetElementType() && source.Type.GetElementType()!.IsPrimitiveKind())
        {
            //Array.Copy(src, 0, dest, 0, src.Length)
            var method = typeof(Array).GetMethod("Copy",
                new[] {typeof(Array), typeof(int), typeof(Array), typeof(int), typeof(int)});
            return Expression.Call(method!, source, Expression.Constant(0), destination, Expression.Constant(0),
    ...
    private Expression CreateArraySet(Expression source, Expression destination, CompileArgument arg)
    {
        ...
        var vx = Enumerable.Range(0, destination.Type.GetArrayRank()).Select(i => Expression.Variable(typeof(int), "v" + i))
            .ToList();
        var vlenx = Enumerable.Range(0, destination.Type.GetArrayRank()).Select(i => Expression.Variable(typeof(int), "vlen" + i))
            .ToList();
        var block = new List<Expression>();
        block.AddRange(vx.Select(v => Expression.Assign(v, Expression.Constant(0))));

        var method = typeof(Array).GetMethod("GetLength", new[] {typeof(int)});
        block.AddRange(vlenx.Select((vlen, i) =>
            Expression.Assign(vlen, Expression.Call(destination, method!, Expression.Constant(i)))));
        var getter = CreateAdaptExpression(item, destinationElementType, arg);
        var set = ExpressionEx.Assign(Expression.ArrayAccess(destination, vx), getter);

        Expression ifExpr = Expression.Block(Expression.Assign(vx[1], Expression.Constant(0)),
        ...
        for (var i = 1; i < vx.Count; i++)
        {
            ...
            list.Add(Expression.IfThen(Expression.GreaterThanOrEqual(vx[i], vlenx[i]), ifExpr));
