...
internal class DictionaryAdapter : ClassAdapter
{
    ...
    protected override bool CanInline(Expression source, Expression? destination, CompileArgument arg)
    {
        ...
        return arg.SourceType.GetDictionaryType() == null || arg.Settings.IgnoreNonMapped == true;
    }
    ...
    protected override Expression CreateBlockExpression(Expression source, Expression destination, CompileArgument arg)
    {
        ...
        if (source.Type.IsObjectReference() && destination.Type.IsObjectReference() &&
        ...
        //ignore mapped
        var ignores = arg.Settings.Resolvers.Select(r => r.SourceMemberName).Where(name => name != null).ToHashSet();
        ...
        foreach (var ignore in arg.Settings.Ignore)
        {
            ...
            else
            {
                ...
                var setWithCondition = Expression.IfThen(ExpressionEx.Not(body), set);
        ...
        {
            var cases = ignoreIfs.Select(k => Expression.SwitchCase(k.Value, Expression.Constant(k.Key))).ToList();
            if (ignores.Count > 0)
        ...
        if (arg.Settings.IgnoreNullValues == true && kvpValueType.CanBeNull())
        {
            set = Expression.IfThen(
                Expression.NotEqual(Expression.Property(kvp, "Value"), Expression.Constant(null, kvpValueType)), set);
        }
        ...
        return shouldConvert ? Expression.Block(new[] {(ParameterExpression) dict}, actions) : Expression.Block(actions);
    }
    ...
    protected override ClassModel GetSetterModel(CompileArgument arg)
    {
        ...
        if (arg.SourceType.GetDictionaryType() == null)
        {
            ...
            var propNames = arg.SourceType.GetFieldsAndProperties(true)
                .Where(model => model.ShouldMapMember(arg, MemberSide.Source)).Select(model => model.Name)
        ...
        var sourceModels = destNames.Select(name => new KeyValuePairModel(name, dictArgs[1], getFn, setFn)).ToList();

        return new ClassModel {Members = sourceModels};
    }
    ...
    private static Func<Expression, Expression, Expression> GetFunction(CompileArgument arg, Type dictType)
    {
        ...
        {
            var getMethod = typeof(MapsterHelper).GetMethods().First(m =>
    ...
    {
        var strategy = arg.Settings.NameMatchingStrategy;
        if (arg.MapType == MapType.MapToTarget && strategy.DestinationMemberNameConverter != MapsterHelper.Identity)
        {
            var args = dictType.GetGenericArguments();
            var setMethod = typeof(MapsterHelper).GetMethods().First(m => m.Name == nameof(MapsterHelper.FlexibleSet))
                .MakeGenericMethod(args[1]);
            var destNameConverter = ExpressionEx.GetNameConverterExpression(strategy.DestinationMemberNameConverter);
