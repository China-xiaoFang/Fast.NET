...
public class ArrayAdapter : BaseAdapter
{
    ...
    {
        return arg.SourceType.IsCollection() && arg.DestinationType.IsArray;
    }
    ...
    {
        if (source.Type.IsArray && source.Type.GetArrayRank() == 1 &&
            source.Type.GetElementType() == destination.Type.GetElementType() && source.Type.GetElementType()!.IsPrimitiveKind())
        {
            ...
            var len = arg.UseDestinationValue
                ? Expression.Call(typeof(Math).GetMethod("Min", new[] {typeof(int), typeof(int)})!,
                    ExpressionEx.CreateCountExpression(source)!, ExpressionEx.CreateCountExpression(destination)!)
    ...
    protected override Expression CreateInlineExpression(Expression source, CompileArgument arg)
    {
        ...
        var toList = (from m in typeof(Enumerable).GetMethods() where m.Name == nameof(Enumerable.ToArray) select m).First()
    ...
    private Expression CreateArraySet(Expression source, Expression destination, CompileArgument arg)
    {
        ...
        var set = Expression.Assign(Expression.ArrayAccess(destination, Expression.PostIncrementAssign(v)), getter);
