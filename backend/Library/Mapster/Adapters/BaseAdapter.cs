...
public abstract class BaseAdapter
{
    ...
    public LambdaExpression CreateAdaptFunc(CompileArgument arg)
    {
        ...
        return Expression.Lambda(body, new[] {p}.Concat(arg.Context.ExtraParameters)).TrimParameters(1);
    }
    ...
    public LambdaExpression CreateAdaptToTargetFunc(CompileArgument arg)
    {
        ...
        return Expression.Lambda(body, new[] {p, p2}.Concat(arg.Context.ExtraParameters)).TrimParameters(2);
    }
    ...
    public TypeAdapterRule CreateRule()
    {
        var rule = new TypeAdapterRule
        {
            ...
            {
                ConverterFactory = CreateAdaptFunc, ConverterToTargetFactory = CreateAdaptToTargetFunc,
            }
        };
        DecorateRule(rule);
        return rule;
    ...
    protected virtual bool CanInline(Expression source, Expression? destination, CompileArgument arg)
    {
        ...
        if (arg.Settings.PreserveReference == true && arg.SourceType.IsObjectReference() &&
    ...
    protected virtual Expression CreateExpressionBody(Expression source, Expression? destination, CompileArgument arg)
    {
        if (CheckExplicitMapping && arg.Context.Config.RequireExplicitMapping && !arg.ExplicitMapping)
            throw new InvalidOperationException(
                "Implicit mapping is not allowed (check GlobalSettings.RequireExplicitMapping) and no configuration exists");
        ...
        try
        {
            ...
            if (arg.Context.Running.Count > 1 && !arg.Context.Config.SelfContainedCodeGeneration && !arg.UseDestinationValue &&
                !arg.Context.IsSubFunction())
            {
                if (destination == null)
                    return arg.Context.Config.CreateMapInvokeExpressionBody(source.Type, arg.DestinationType, source);
                else
                    return arg.Context.Config.CreateMapToTargetInvokeExpressionBody(source.Type, arg.DestinationType, source,
                        destination);
            }
            else
    ...
    protected Expression CreateBlockExpressionBody(Expression source, Expression? destination, CompileArgument arg)
    {
        ...
        foreach (var tuple in arg.Settings.Includes)
        {
            ...
            var drvdSourceAssign = Expression.Assign(drvdSource, Expression.TypeAs(source, tuple.Source));
            ...
            if (destination != null)
            {
                ...
                var drvdDestAssign = Expression.Assign(drvdDest, Expression.TypeAs(destination, tuple.Destination));
                blocks.Add(drvdDestAssign);
                cond = Expression.AndAlso(cond, Expression.NotEqual(drvdDest, Expression.Constant(null, tuple.Destination)));
            }
        ...
        else
        {
            ...
            if (source.CanBeNull())
            {
                ...
                blocks.Add(Expression.IfThen(compareNull, Expression.Return(label, arg.DestinationType.CreateDefault())));
            }
            ...
            var beforeMappings = arg.Settings.BeforeMappingFactories
                .Select(it => InvokeMapping(it, source, result, destination, arg, true)).Reverse();
            ...
            var afterMappings = arg.Settings.AfterMappingFactories
                .Select(it => InvokeMapping(it, source, result, destination, arg, false)).Reverse();
                ...
                arg.DestinationType.IsObjectReference())
            {
                ...
                var assignKey = Expression.Assign(key,
                    Expression.New(tupleType.GetConstructor(new[] {typeof(object), typeof(Type)})!, source,
                        Expression.Constant(arg.DestinationType)));

                var cache = Expression.Variable(typeof(object), "cache");
                var tryGetMethod =
                    dictType.GetMethod("TryGetValue", new[] {typeof(ReferenceTuple), typeof(object).MakeByRefType()});
                var checkHasRef = Expression.Call(references, tryGetMethod!, key, cache);
                var setResult = Expression.IfThen(checkHasRef, Expression.Return(label, cache.To(arg.DestinationType)));

                var indexer = dictType.GetProperties().First(item => item.GetIndexParameters().Length > 0);
                var refAssign = Expression.Assign(Expression.Property(references, indexer, key),
                    Expression.Convert(result, typeof(object)));
                assignActions.Add(refAssign);

                var usingBody = Expression.Block(new[] {cache, references, key, result},
                    new Expression[] {assignReferences, assignKey, setResult}.Concat(assignActions).Concat(settingActions));
    ...
    private static Expression InvokeMapping(Func<CompileArgument, LambdaExpression> mappingFactory, Expression source,
        Expression result, Expression? destination, CompileArgument arg, bool setResult)
    {
    ...
    protected virtual Expression CreateInstantiationExpression(Expression source, Expression? destination, CompileArgument arg)
    {
        ...
        if (constructUsing != null)
        {
            ...
            return constructUsing.Apply(arg.MapType, args).TrimConversion(true).To(arg.DestinationType);
        }

        //if there is default constructor, use default constructor

        if (arg.DestinationType.HasDefaultConstructor())
        {
        ...
        if (destination != null || arg.Settings.Includes.Count > 0)
        {
            return Expression.Throw(Expression.New(
                // ReSharper disable once AssignNullToNotNullAttribute
                typeof(InvalidOperationException).GetConstructor(new[] {typeof(string)}),
                Expression.Constant("Cannot instantiate type: " + arg.DestinationType.Name)), arg.DestinationType);
        }
    ...
    private static Expression CreateAdaptExpressionCore(Expression source, Type destinationType, CompileArgument arg,
        MemberMapping? mapping = null, Expression? destination = null)
    {
        var mapType = arg.MapType == MapType.MapToTarget && destination == null ? MapType.Map :
            mapping?.UseDestinationValue == true ? MapType.MapToTarget : arg.MapType;
        ...
        try
        {
            ...
            var lambda = arg.Context.Config.CreateInlineMapExpression(source.Type, destinationType, mapType, arg.Context,
                mapping);
    ...
    internal Expression CreateAdaptExpression(Expression source, Type destinationType, CompileArgument arg,
        MemberMapping? mapping, Expression? destination = null)
    {
