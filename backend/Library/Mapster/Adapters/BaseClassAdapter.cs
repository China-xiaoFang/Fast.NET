...
internal abstract class BaseClassAdapter : BaseAdapter
{
        ...
        Expression? destination = null)
    {
        ...
        sources.AddRange(arg.Settings.ExtraSources.Select(src =>
        ...
        foreach (var destinationMember in destinationMembers)
        {
            ...
            var getter =
                (from fn in resolvers from src in sources select fn(src, destinationMember, arg)).FirstOrDefault(result =>
                    result != null);

            var nextIgnore = arg.Settings.Ignore.Next((ParameterExpression) source, (ParameterExpression?) destination,
                destinationMember.Name);
            var nextResolvers = arg.Settings.Resolvers
                .Next(arg.Settings.Ignore, (ParameterExpression) source, destinationMember.Name).ToList();
            ...
            if (getter != null)
            {
                propertyModel.Getter = arg.MapType == MapType.Projection ? getter : getter.ApplyNullPropagation();
                properties.Add(propertyModel);
            }
            else
            {
                if (arg.Settings.IgnoreNonMapped != true && arg.Settings.Unflattening == true &&
                    arg.DestinationType.GetDictionaryType() == null && arg.SourceType.GetDictionaryType() == null)
                {
        ...
        if (requireDestMemberSource && unmappedDestinationMembers.Count > 0 && arg.Settings.SkipDestinationMemberCheck != true)
        {
            throw new InvalidOperationException(
                $"The following members of destination class {arg.DestinationType} do not have a corresponding source member mapped or ignored:{string.Join(",", unmappedDestinationMembers)}");
        }

        return new ClassMapping {ConstructorInfo = classModel.ConstructorInfo, Members = properties,};
    }

    protected static bool ProcessIgnores(CompileArgument arg, IMemberModel destinationMember,
        out IgnoreDictionary.IgnoreItem ignore)
    {
        ...
        return arg.Settings.Ignore.TryGetValue(destinationMember.Name, out ignore) && ignore.Condition == null;
    }
    ...
    {
        return new ClassModel {Members = arg.DestinationType.GetFieldsAndProperties(true)};
    }
