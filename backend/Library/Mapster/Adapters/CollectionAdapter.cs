...
internal class CollectionAdapter : BaseAdapter
{
    ...
    protected override bool CanInline(Expression source, Expression? destination, CompileArgument arg)
    {
        ...
        if (arg.MapType == MapType.Projection)
        {
            ...
            throw new InvalidOperationException(
                $"{arg.DestinationType} is not supported for projection, please consider using List<>");
        }
    ...
    protected override Expression CreateInlineExpression(Expression source, CompileArgument arg)
    {
        ...
        var adapt = CreateAdaptExpression(p1, destinationElementType, arg).To(destinationElementType, true);
        if (adapt == p1)
        {
            if (arg.MapType == MapType.Projection)
                return source;
            ...
            var toEnum =
                (from m in typeof(MapsterHelper).GetMethods() where m.Name == nameof(MapsterHelper.ToEnumerable) select m).First()
        ...
        {
            var cast = (from m in typeof(Enumerable).GetMethods() where m.Name == nameof(Enumerable.Cast) select m).First()
    ...
    private static Expression InlineChangeType(Expression exp, CompileArgument arg)
    {
        ...
        {
            var toList = (from m in typeof(Enumerable).GetMethods() where m.Name == nameof(Enumerable.ToList) select m).First()
                .MakeGenericMethod(destinationElementType);
            return Expression.Call(toList, exp);
        }

        // if (arg.DestinationType.IsAssignableFromSet())
        var set = typeof(HashSet<>).MakeGenericType(destinationElementType);
        var ctor = (from c in set.GetConstructors()
            let a = c.GetParameters()
            where a.Length == 1 && a[0].ParameterType.GetTypeInfo().IsGenericType &&
    ...
    private Expression CreateListSet(Expression source, Expression destination, CompileArgument arg)
    {
        ...
        var set = Expression.Call(destination, addMethod!, getter);
