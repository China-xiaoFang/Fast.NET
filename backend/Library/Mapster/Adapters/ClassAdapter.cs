...
internal class ClassAdapter : BaseClassAdapter
{
    ...
    protected override bool CanInline(Expression source, Expression? destination, CompileArgument arg)
    {
        ...
        if (constructUsing != null && constructUsing.Body.NodeType != ExpressionType.New &&
    ...
    protected override Expression CreateInstantiationExpression(Expression source, Expression? destination, CompileArgument arg)
    {
        ...
        if (ctor == null)
        {
            ...
            if (destType == null)
                return base.CreateInstantiationExpression(source, destination, arg);
            classConverter = destType.GetConstructors().OrderByDescending(it => it.GetParameters().Length)
                .Select(it => GetConstructorModel(it, true)).Select(it => CreateClassConverter(source, it, arg))
    ...
    protected override Expression CreateBlockExpression(Expression source, Expression destination, CompileArgument arg)
    {
                ...
                if (arg.Settings.IgnoreNullValues == true && member.Getter.CanBeNull())
                {
                    if (adapt is ConditionalExpression condEx)
                    {
                        if (condEx.Test is BinaryExpression {NodeType: ExpressionType.Equal} binEx &&
                            binEx.Left == member.Getter && binEx.Right is ConstantExpression {Value: null})
        ...
        if (conditions != null)
        {
            ...
            {
                var condition = Expression.IfThen(ExpressionEx.Not(kvp.Value.Item2), Expression.Block(kvp.Value.Item1));
                lines.Add(condition);
            }
        }

        return lines.Count > 0 ? Expression.Block(lines) : Expression.Empty();
    }

    private static Expression SetValueByReflection(Expression destination, MemberExpression adapt, Type destinationType)
    {
        ...
        var getPropertyExpression = Expression.Call(typeofExpression, getPropertyMethod, Expression.Constant(memberName));
        var setValueMethod = typeof(PropertyInfo).GetMethod("SetValue", new[] {typeof(object), typeof(object)})!;
        var memberAsObject = adapt.To(typeof(object));
        return Expression.Call(getPropertyExpression, setValueMethod, new[] {destination, memberAsObject});
    }
    ...
    protected override Expression? CreateInlineExpression(Expression source, CompileArgument arg)
    {
        ...
        foreach (var member in members)
        {
            ...
            if (arg.MapType == MapType.Projection && member.Getter.Type != member.DestinationMember.Type &&
                !member.Getter.Type.IsCollection() && !member.DestinationMember.Type.IsCollection() && member.Getter.Type
                    .GetTypeInfo().GetCustomAttributesData().All(attr => attr.GetAttributeType().Name != "ComplexTypeAttribute"))
            {
