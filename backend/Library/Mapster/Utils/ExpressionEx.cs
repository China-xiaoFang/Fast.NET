...
internal static class ExpressionEx
{
    ...
    {
        var middle = left.Type.IsReferenceAssignableFrom(right.Type) ? right : Expression.Convert(right, left.Type);
    ...
    private static Expression PropertyOrField(Expression expr, string prop)
    {
        ...
        if (dictType?.GetGenericArguments()[0] == typeof(string))
        {
            var method = typeof(MapsterHelper).GetMethods()
                .First(m => m.Name == nameof(MapsterHelper.GetValueOrDefault) &&
                            m.GetParameters()[0].ParameterType.Name == dictType.Name)
        ...
        if (type.GetTypeInfo().IsInterface)
        {
            ...
            var interfaceType =
                allTypes.FirstOrDefault(it => it.GetProperty(prop, flags) != null || it.GetField(prop, flags) != null);
    ...
    public static Delegate Compile(this LambdaExpression exp, CompileArgument arg)
    {
        ...
        {
            throw new CompileException(arg, ex) {Expression = exp,};
        }
    }
    ...
    public static Expression? CreateCountExpression(Expression source)
    {
        ...
        {
            return source.Type.GetArrayRank() == 1 ? Expression.ArrayLength(source) : Expression.Property(source, "Length");
        }

        var countProperty = GetCount(source.Type);
        return countProperty != null ? Expression.Property(source, countProperty) : null;
    }
    ...
    private static PropertyInfo? GetCount(Type type)
    {
        ...
        return collection.IsAssignableFrom(type) ? collection.GetProperty("Count") : null;
    }
    ...
    public static Expression ForLoop(Expression collection, ParameterExpression loopVar, params Expression[] loopContent)
    {
        ...
        return Expression.Block(new[] {i, len}, iAssign, lenAssign,
            Expression.Loop(
                Expression.IfThenElse(Expression.LessThan(i, len),
                    Expression.Block(new[] {loopVar},
                        new[] {Expression.Assign(loopVar, current)}.Concat(loopContent)
                            .Concat(new[] {Expression.PostIncrementAssign(i)})), Expression.Break(breakLabel)), breakLabel));
    }
    ...
    public static Expression ForEach(Expression collection, ParameterExpression loopVar, params Expression[] loopContent)
    {
        ...
        return Expression.Block(new[] {enumeratorVar}, enumeratorAssign,
            Expression.Loop(
                Expression.IfThenElse(moveNextCall,
                    Expression.Block(new[] {loopVar}, new[] {Assign(loopVar, current)}.Concat(loopContent)),
    ...
    public static Expression NotNullReturn(this Expression exp, Expression value)
    {
        ...
        return Expression.Condition(compareNull, value.Type.CreateDefault(), value);
    }
