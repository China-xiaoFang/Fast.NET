...
internal static class ReflectionUtils
{
    ...
    public static bool IsPoco(this Type type)
    {
        ...
        return type.GetFieldsAndProperties()
            .Any(it => (it.SetterModifier & (AccessModifier.Public | AccessModifier.NonPublic)) != 0);
    }
    ...
    public static IEnumerable<IMemberModelEx> GetFieldsAndProperties(this Type type, bool includeNonPublic = false)
    {
        ...
        IEnumerable<IMemberModelEx> GetPropertiesFunc(Type t) =>
            t.GetProperties(bindingFlags).Where(x => x.GetIndexParameters().Length == 0).Select(CreateModel);

        IEnumerable<IMemberModelEx> GetFieldsFunc(Type t) => t.GetFields(bindingFlags).Select(CreateModel);
    }
    ...
    public static Type ExtractCollectionType(this Type collectionType)
    {
        ...
        return enumerableType != null ? enumerableType.GetGenericArguments()[0] : typeof(object);
    }
    ...
    public static bool IsRecordType(this Type type)
    {
        ...
        if (type.GetTypeInfo().IsInterface && props.Any(p => p.SetterModifier != AccessModifier.Public))
            return true;
    ...
    {
#if !NET40
        if (destinationType.GetTypeInfo().IsInterface && destinationType.GetTypeInfo().IsGenericType &&
    ...
    public static bool ShouldMapMember(this IMemberModel member, CompileArgument arg, MemberSide side)
    {
        ...
        var result = predicates.Select(predicate => predicate(member, side)).FirstOrDefault(it => it != null);
        if (result != null)
            return result == true;
    ...
    public static string GetMemberName(this IMemberModel member, MemberSide side,
        List<Func<IMemberModel, MemberSide, string?>> getMemberNames, Func<string, string> nameConverter)
    {
        var memberName = getMemberNames.Select(func => func(member, side)).FirstOrDefault(name => name != null) ?? member.Name;
    ...
    {
        return type.CanBeNull() ? Expression.Constant(null, type) : Expression.Constant(Activator.CreateInstance(type), type);
    }
    ...
    {
        if (type == typeof(void) || type.GetTypeInfo().IsAbstract || type.GetTypeInfo().IsInterface)
            return false;
    ...
    public static IEnumerable<InvokerModel> Next(this IEnumerable<InvokerModel> resolvers, IgnoreDictionary ignore,
        ParameterExpression source, string destName)
    {
        return resolvers.Where(it => !ignore.TryGetValue(it.DestinationMemberName, out var item) || item.Condition != null)
            .Select(it => it.Next(source, destName)).Where(it => it != null)!;
    }
