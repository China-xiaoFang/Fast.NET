...
public class TypeAdapterConfig
{
        ...
        return new List<TypeAdapterRule>
        {
            ...
            new TypeAdapterRule
            {
                ...
                Settings = new TypeAdapterSettings
                {
                    ...
                    UseDestinationValues = {UseDestinationValue.Attribute,},
            ...
            {
                Priority =
                    arg => arg.SourceType.GetDictionaryType()?.GetGenericArguments()[0] == typeof(string)
                        ? DictionaryAdapter.DefaultScore
                        : null,
                Settings = new TypeAdapterSettings
                {
                    ValueAccessingStrategies =
                    {
                        ValueAccessingStrategy.CustomResolverForDictionary, ValueAccessingStrategy.Dictionary,
                    },
                }
            }
        };
    ...
    public ConcurrentDictionary<TypeTuple, TypeAdapterRule> RuleMap { get; internal set; } =
        new ConcurrentDictionary<TypeTuple, TypeAdapterRule>();
    ...
    public TypeAdapterConfig()
    {
        ...
        Rules.Add(new TypeAdapterRule {Priority = arg => -100, Settings = settings,});
    }
    ...
    public TypeAdapterSetter When(Func<Type, Type, MapType, bool> canMap)
    {
        ...
        {
            Priority = arg => canMap(arg.SourceType, arg.DestinationType, arg.MapType) ? 25 : null,
            Settings = new TypeAdapterSettings(),
        };
        Rules.LockAdd(rule);
    ...
    {
        var rule = new TypeAdapterRule {Priority = arg => canMap(arg) ? 25 : null, Settings = new TypeAdapterSettings(),};
        Rules.LockAdd(rule);
        return new TypeAdapterSetter(rule.Settings, this);
    }
    ...
    private TypeAdapterSettings GetSettings(TypeTuple key)
    {
        ...
        {
            var r = types.Source == typeof(void) ? CreateDestinationTypeRule(types) : CreateTypeTupleRule(types);
            Rules.LockAdd(r);
            return r;
        });
    ...
    private static int? GetSubclassDistance(Type type1, Type type2, bool allowInheritance)
    {
        ...
        if (type2.GetTypeInfo().IsGenericTypeDefinition)
        {
            ...
            return type1 != null && type1.GetTypeInfo().IsGenericType && type1.GetGenericTypeDefinition() == type2 ? score : null;
        }
    ...
    private readonly ConcurrentDictionary<TypeTuple, MethodCallExpression> _projectionDict =
        new ConcurrentDictionary<TypeTuple, MethodCallExpression>();
    ...
    {
        var fn = arg.MapType == MapType.MapToTarget ? arg.Settings.ConverterToTargetFactory : arg.Settings.ConverterFactory;
        if (fn == null)
    ...
    private LambdaExpression CreateDynamicMapExpression(TypeTuple tuple)
    {
        ...
        return Expression.Lambda(Expression.Block(new[] {pOld}, assign, lambda.Body), pNew);
    }

    internal LambdaExpression CreateInlineMapExpression(Type sourceType, Type destinationType, MapType mapType,
        CompileContext context, MemberMapping? mapping = null)
    {
            ...
            if (context.Running.Contains(tuple))
            {
                if (mapType == MapType.Projection)
                    throw new InvalidOperationException(
                        "Projection does not support circular reference, please use MaxDepth setting");
    ...
    private IEnumerable<TypeAdapterRule> GetAttributeSettings(TypeTuple tuple, MapType mapType)
    {
        var rules1 = from type in tuple.Source.GetAllTypes()
            ...
            select new TypeAdapterRule {Priority = arg => distance + 50, Settings = CreateSettings(attr)};
    ...
    public IList<IRegister> Scan(params Assembly[] assemblies)
    {
        var registers = assemblies.Select(assembly => assembly.GetLoadableTypes()
            .Where(x => typeof(IRegister).GetTypeInfo().IsAssignableFrom(x.GetTypeInfo()) && x.GetTypeInfo().IsClass &&
                        !x.GetTypeInfo().IsAbstract)).SelectMany(registerTypes =>
    ...
    private static readonly Lazy<TypeAdapterConfig> _cloneConfig = new Lazy<TypeAdapterConfig>(() =>
    {
        ...
        config.ForType<TypeAdapterSettings, TypeAdapterSettings>().MapWith(src => src.Clone(), true);
        return config;
    });
