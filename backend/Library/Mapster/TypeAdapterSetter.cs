...
public static class TypeAdapterSetterExtensions
{
        ...
        Expression<Func<TDestinationMember, TDestinationMember>> transform) where TSetter : TypeAdapterSetter
    {
        ...
        {
            Condition = t => t == typeof(TDestinationMember), TransformFunc = _ => transform,
        });
        return setter;
    }
    ...
    public static TSetter IgnoreAttribute<TSetter>(this TSetter setter, params Type[] types) where TSetter : TypeAdapterSetter
    {
        ...
        {
            setter.Settings.ShouldMapMember.Add((member, _) => member.HasCustomAttribute(type) ? false : null);
        }
    ...
    public static TSetter IncludeAttribute<TSetter>(this TSetter setter, params Type[] types) where TSetter : TypeAdapterSetter
    {
        ...
        foreach (var type in types)
        {
            setter.Settings.ShouldMapMember.Add((member, _) => member.HasCustomAttribute(type) ? true : null);
        }
        ...
        where TSetter : TypeAdapterSetter
    {
        ...
        setter.Settings.ShouldMapMember.Add((member, side) => predicate(member, side) ? false : null);
        return setter;
    }

    public static TSetter IncludeMember<TSetter>(this TSetter setter, Func<IMemberModel, MemberSide, bool> predicate)
        where TSetter : TypeAdapterSetter
    {
        setter.CheckCompiled();

        setter.Settings.ShouldMapMember.Add((member, side) => predicate(member, side) ? true : null);
        return setter;
    }
    ...
    public static TSetter Map<TSetter, TSourceMember>(this TSetter setter, string memberName,
        Expression<Func<TSourceMember>> source) where TSetter : TypeAdapterSetter
    {
        ...
        setter.Settings.Resolvers.Add(new InvokerModel {DestinationMemberName = memberName, Invoker = invoker, Condition = null});

        return setter;
    }

    public static TSetter Map<TSetter, TSource, TSourceMember>(this TSetter setter, string memberName,
    ...
    public static TSetter Map<TSetter>(this TSetter setter, string destinationMemberName, string sourceMemberName)
        where TSetter : TypeAdapterSetter
    {
        setter.CheckCompiled();
        ...
        setter.Settings.Resolvers.Add(new InvokerModel
        {
            DestinationMemberName = destinationMemberName, SourceMemberName = sourceMemberName, Condition = null
        });
        ...
        where TSetter : TypeAdapterSetter
    {
        ...
        {
            Priority = arg => arg.SourceType == sourceType && arg.DestinationType == destType ? 100 : null,
        ...
        where TSetter : TypeAdapterSetter
    {
        ...
        {
            setter.IgnoreMember((member, _) => !member.GetCustomAttributesData().Select(it => it.GetAttributeType())
                .Intersect(attr.IgnoreNoAttributes).Any());
        }
...
{
    internal TypeAdapterSetter(TypeAdapterSettings settings, TypeAdapterConfig parentConfig) : base(settings, parentConfig)
    {
    }
    ...
    public TypeAdapterSetter<TDestination> Map<TDestinationMember, TSourceMember>(
        Expression<Func<TDestination, TDestinationMember>> member, Expression<Func<TSourceMember>> source)
    {
        this.CheckCompiled();
        ...
        {
            DestinationMemberName = member.GetMemberPath()!, Invoker = invoker, Condition = null
        });
        return this;
    }

    public TypeAdapterSetter<TDestination> Map<TDestinationMember>(
        Expression<Func<TDestination, TDestinationMember>> destinationMember, string sourceMemberName)
    {
        this.CheckCompiled();
        ...
        Settings.Resolvers.Add(new InvokerModel
        {
            DestinationMemberName = destinationMember.GetMemberPath()!, SourceMemberName = sourceMemberName, Condition = null
        });
    ...
    public TypeAdapterSetter<TDestination> BeforeMappingInline(Expression<Action<TDestination>> action)
    {
        ...
        var lambda = Expression.Lambda(action.Body, Expression.Parameter(typeof(object), SourceParameterName),
            action.Parameters[0]);
...
public class TypeAdapterSetter<TSource, TDestination> : TypeAdapterSetter<TDestination>
{
    ...
    public TypeAdapterSetter<TSource, TDestination> IgnoreIf(Expression<Func<TSource, TDestination, bool>> condition,
    ...
    public TypeAdapterSetter<TSource, TDestination> Map<TDestinationMember, TSourceMember>(
        Expression<Func<TDestination, TDestinationMember>> member, Expression<Func<TSource, TSourceMember>> source,
    ...
    public TypeAdapterSetter<TSource, TDestination> Map<TSourceMember>(string memberName,
    ...
    public TypeAdapterSetter<TSource, TDestination> Include<TDerivedSource, TDerivedDestination>()
        where TDerivedSource : class, TSource where TDerivedDestination : class, TDestination
    {
    ...
    public TypeAdapterSetter<TSource, TDestination> Inherits<TBaseSource, TBaseDestination>()
    {
        ...
        var baseSourceType = typeof(TBaseSource);
        var baseDestinationType = typeof(TBaseDestination);
...
public class TwoWaysTypeAdapterSetter<TSource, TDestination>
{
    ...
    public TwoWaysTypeAdapterSetter<TSource, TDestination> Map<TSourceMember>(string memberName,
