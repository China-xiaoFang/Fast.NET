...
public class InvokerModel
{
    ...
    public InvokerModel? Next(ParameterExpression source, string destMemberName)
    {
        ...
        return new InvokerModel
        {
            ...
            Condition = IsChildPath || Condition == null ? Condition : Expression.Lambda(Condition.Apply(source), source),
            Invoker = IsChildPath ? Invoker : Expression.Lambda(GetInvokingExpression(source), source),
    ...
    public Expression GetInvokingExpression(Expression exp, MapType mapType = MapType.Map)
    {
        if (IsChildPath)
            return Invoker!.Body;
        return SourceMemberName != null ? ExpressionEx.PropertyOrFieldPath(exp, SourceMemberName) : Invoker!.Apply(mapType, exp);
    }

    public Expression? GetConditionExpression(Expression exp, MapType mapType = MapType.Map)
    {
        return IsChildPath ? Condition?.Body : Condition?.Apply(mapType, exp);
    }
}