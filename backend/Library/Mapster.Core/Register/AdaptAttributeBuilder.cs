...
public class AdaptAttributeBuilder
{
    ...
    public Dictionary<Type, Dictionary<string, PropertySetting>> TypeSettings { get; } =
        new Dictionary<Type, Dictionary<string, PropertySetting>>();
    ...
    public AdaptAttributeBuilder ForTypes(params Type[] types)
    {
        foreach (var type in types)
        {
            if (!TypeSettings.ContainsKey(type))
                TypeSettings.Add(type, new Dictionary<string, PropertySetting>());
        }
    ...
    public AdaptAttributeBuilder ForAllTypesInNamespace(Assembly assembly, string @namespace)
    {
        foreach (var type in assembly.GetTypes())
        {
            if ((type.Namespace == @namespace || type.Namespace?.StartsWith(@namespace + '.') == true) &&
                !type.Name.Contains('<') && !TypeSettings.ContainsKey(type))
                TypeSettings.Add(type, new Dictionary<string, PropertySetting>());
        }
    ...
    {
        if (!TypeSettings.TryGetValue(typeof(T), out var settings))
        {
            settings = new Dictionary<string, PropertySetting>();
            TypeSettings.Add(typeof(T), settings);
        }
    ...
    {
        foreach (var type in TypeSettings.Keys.ToList())
        {
            if (predicate(type))
                TypeSettings.Remove(type);
        }
    ...
    {
        Attribute.IgnoreAttributes = attributes;
        return this;
    }
    ...
    {
        Attribute.IgnoreNoAttributes = attributes;
        return this;
    }
    ...
    {
        Attribute.IgnoreNamespaces = namespaces;
        return this;
    }
    ...
    {
        Attribute.IgnoreNullValues = value;
        return this;
    }
    ...
    {
        Attribute.RequireDestinationMemberSource = value;
        return this;
    }
    ...
    {
        Attribute.MapToConstructor = value;
        return this;
    }
    ...
    {
        Attribute.PreserveReference = value;
        return this;
    }
    ...
    {
        Attribute.ShallowCopyForSameType = value;
        return this;
    }
    ...
    {
        AlterTypes.Add(type => type == typeof(TFrom) ? typeof(TTo) : null);
        return this;
    }
    ...
    {
        AlterTypes.Add(type => predicate(type) ? toType : null);
        return this;
    }
}